	...
	void f_debug_info(void)
	{
		...
		switch (arg[0].u.number)
		{
			...
			case 1:
				...
				outbuf_addv(&out, "runtime fun table %d (%d) \n", ob->prog->num_functions_total, ob->prog->num_functions_total *(sizeof(runtime_function_u) + 1) );
				outbuf_addv(&out, "compiler fun table %d (%d) \n", ob->prog->num_functions_defined, ob->prog->num_functions_defined *sizeof(compiler_function_t) );
				outbuf_addv(&out, "num strings %d\n", ob->prog->num_strings);
				outbuf_addv(&out, "num vars %d (%d)\n", ob->prog->num_variables_defined, ob->prog->num_variables_defined *(sizeof(char*) + sizeof(short) ) );
				outbuf_addv(&out, "num inherits %d (%d)\n", ob->prog->num_inherited, ob->prog->num_inherited *sizeof(inherit_t) );
 ...
 * defined(DEBUGMALLOC_EXTENSIONS)) */
	...
	void f_traceprefix(void)
	{
		...
		if (command_giver && command_giver->interactive)
		{
			...
			if (sp->type &T_STRING)
			{
				command_giver->interactive->trace_prefix = make_shared_string(sp->u.string);
				free_string_svalue(sp);
			}
			else
			{
