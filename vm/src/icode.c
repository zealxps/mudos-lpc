 ...
 */
static void ins_short(int _l)
{
	unsigned short l = _l;
	if (_l > 0xffff) {
		debug_message("QTZ_ServerEngine:ins_short too big:%d\n", _l);
		dump_trace(0);
		abort();
		return ;
	}
...
static unsigned short read_short(int offset)
{
	unsigned short l;
...
static void upd_short(int offset, int _l)
{
	unsigned short l = _l;
	IF_DEBUG(UPDATE_PROGRAM_SIZE);
	DEBUG_CHECK2(offset > CURRENT_PROGRAM_SIZE, "QTZ_ServerEngine:patch offset %x larger than current program size %x.\n", offset, CURRENT_PROGRAM_SIZE);
	if (_l > 0xffff) {
		debug_message("QTZ_ServerEngine:LPC File is Too Long %d %d\n", l, _l);
		dump_trace(0);
		abort();
		return ;
	}
...
static void initialize_push(void)
{
	...
	if (what != F_CONST0 && what != F_CONST1) {
		arg = mem_block[current_block].block[push_start + 1];
	}
...
INLINE void switch_to_line(int line)
{
...
void i_generate_node(parse_node_t *expr)
{
	...
	switch (expr->kind) {
		...
		case NODE_OPCODE_1:
			if (expr->v.number == F_LOCAL) {
				if (try_to_push(PUSH_LOCAL, expr->l.number) ) {
					break;
				}
			} else if (expr->v.number == F_GLOBAL) {
				if (try_to_push(PUSH_GLOBAL, expr->l.number) ) {
					break;
				}
			} else if (expr->v.number == F_UPVALUE) {
				if (try_to_push(PUSH_UPVALUE, expr->l.number)) {
					break;
				}
			}
			end_pushes();
		...
		case NODE_STRING:
			if (try_to_push(PUSH_STRING, expr->v.number) ) {
				break;
			}
		...
		case NODE_ANON_FUNC:
			{
				...
				// support for upvalue
				if (expr->l.expr) {
					generate_expr_list(expr->l.expr);
					end_pushes();
					ins_byte(F_AGGREGATE);
					ins_short(expr->l.expr->kind);
				} else {
					end_pushes();
					ins_byte(F_CONST0);
				}
				...
				end_pushes();
				ins_byte(F_FUNCTION_CONSTRUCTOR);
				if (expr->v.expr->r.number &0x10000) {
					ins_byte(FP_ANONYMOUS | FP_NOT_BINDABLE);
				} else {
					ins_byte(FP_ANONYMOUS);
				}
				ins_byte(expr->v.expr->r.number &0xff); // arguments
				ins_byte(expr->v.expr->l.number); // locals
				addr = CURRENT_PROGRAM_SIZE;
				ins_short(0);
				i_generate_node(expr->r.expr);
				upd_short(addr, CURRENT_PROGRAM_SIZE - addr - 2);
	...
	default:
		fatal("QTZ_ServerEngine:Unknown node %i in i_generate_node.\n", expr->kind);
...
static void i_generate_loop(int test_first, parse_node_t *block, parse_node_t *inc, parse_node_t *test)
{
	...
	if (test->v.number == F_LOOP_COND_LOCAL || test->v.number == F_LOOP_COND_NUMBER || test->v.number == F_NEXT_FOREACH) {
		i_generate_node(test);
		ins_short(CURRENT_PROGRAM_SIZE - pos);
	} else {
		i_branch_backwards(generate_conditional_branch(test), pos);
	}
...
void i_generate_inherited_init_call(int index, short f)
{
	end_pushes();
...
{
	add_to_mem_block(A_PROGRAM, (char*)mem_block[A_INITIALIZER].block, mem_block[A_INITIALIZER].current_size);
...
void i_branch_backwards(char b, int addr)
{
	end_pushes();
	if (b) {
		if (b != F_WHILE_DEC) {
...
static void i_update_branch_list(parse_node_t *bl)
{
	int current_size;
...
void i_generate_final_program(int x)
{
	if (!x) {
		UPDATE_PROGRAM_SIZE;
		/* This needs work
				...
				case F_LAND:
					{
						...
						if (tmp > pc) {
							...
							#ifdef DEBUG
								fprintf(stderr, "QTZ_ServerEngine:Optimization failed; can't || or && backwards.\n");
							#endif 
							pc += 2;
							break;
						}
		...
		case F_NBYTE:
			...
			case F_FUNCTION_CONSTRUCTOR:
				...
				case FP_LOCAL:
					...
					break;
				case FP_FUNCTIONAL:
					case FP_FUNCTIONAL | FP_NOT_BINDABLE: pc += 3;
					break;
				case FP_ANONYMOUS:
					case FP_ANONYMOUS | FP_NOT_BINDABLE: pc += 4;
					break;
				case FP_EFUN:
					pc += 2;
					break;
			...
			case F_SWITCH:
				{
					...
					pc += 2; /* def */
					DEBUG_CHECK(stable < pc - start || etable < pc - start || etable < stable, "QTZ_ServerEngine:Error in switch table found while optimizing\n");
			...
			default:
				if ( (instr >= BASE) && (instrs[instr].min_arg != instrs[instr].max_arg) ) {
					pc++;
				}
		}
	}
