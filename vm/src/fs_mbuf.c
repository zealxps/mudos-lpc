...
void fs_mbuf_reset(fs_mbuf_t *mbuf, size_t reset_size)
{
	// fs_mbuf_blk_t *blk, *tmp;

	if (mbuf->blk_count > 1 || (reset_size > mbuf->alloc_size)) {
		fs_mbuf_destruct(mbuf);
		fs_mbuf_init(mbuf, (reset_size > mbuf->alloc_size) ? reset_size : mbuf->alloc_size);
	} else {
...
{
	fs_net_mbuf_t *netbuf = (fs_net_mbuf_t *)calloc(1, sizeof(fs_net_mbuf_t));
...
void fs_mbuf_drain(fs_mbuf_t *mbuf, unsigned drainlen)
{
	fs_mbuf_deq(mbuf, NULL, drainlen);
}
...
{
	fs_free_array_entry_t *entry = (fs_free_array_entry_t *)element;
	entry->next = farr->free;
	farr->free = entry;
	farr->element_alloc--;
