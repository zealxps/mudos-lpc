...
parse_node_t *binary_int_op(parse_node_t *l, parse_node_t *r, char op, char *name)
{
	parse_node_t *ret;
	...
	if (exact_types)
	{
		if (!IS_TYPE(l->type, TYPE_NUMBER) )
		{
			...
			p = strput(buf, end, "QTZ_ServerEngine:Bad left argument to '");
			p = strput(p, end, name);
			p = strput(p, end, "' : \"");
			p = _get_type_name(p, end, l->type);
			p = strput(p, end, "\"");
			yyerror(buf);
		}
		if (!IS_TYPE(r->type, TYPE_NUMBER) )
		{
			...
			p = strput(buf, end, "QTZ_ServerEngine:Bad right argument to '");
			p = strput(p, end, name);
			p = strput(p, end, "' : \"");
			p = _get_type_name(p, end, r->type);
			p = strput(p, end, "\"");
			yyerror(buf);
		}
	}
		...
		if (r->kind == NODE_NUMBER)
		{
			switch (op)
			{
				...
				case F_MOD:
					if (r->v.number == 0)
					{
						yyerror("QTZ_ServerEngine:Modulo by zero constant");
						break;
					}
					l->v.number %= r->v.number;
					break;
				default:
					fatal("QTZ_ServerEngine:Unknown opcode in binary_int_op()\n");
			}
			return l;
		}
		switch (op)
	...
	if (exact_types)
	{
		switch (expr->type)
		{
			...
			default:
				...
				{
					type_error("QTZ_ServerEngine:Bad type of argument used for range: ", expr->type);
		...
		if (!IS_TYPE(l->type, TYPE_NUMBER) )
		{
			type_error("QTZ_ServerEngine:Bad type of left index to range operator", l->type);
		}
		if (r && !IS_TYPE(r->type, TYPE_NUMBER) )
		{
			type_error("QTZ_ServerEngine:Bad type of right index to range operator", r->type);
		}
	}
	else
	{
...
parse_node_t *insert_pop_value(parse_node_t *expr)
{
	...
	switch (expr->kind)
	{
		...
		case NODE_OPCODE_1:
			...
			case F_LT:
			case F_LE:
				yywarn("QTZ_ServerEngine:Value of conditional expression is unused");
				/* FALLTHRU */
			case F_OR:
			case F_XOR:
			case F_AND:
...
parse_node_t *optimize_loop_test(parse_node_t *pn)
{
	...
	else if (IS_NODE(pn, NODE_UNARY_OP, F_POST_DEC) && IS_NODE(pn->r.expr, NODE_OPCODE_1, F_LOCAL_LVALUE) )
	{
