/*
 * sprintf.c v1.05 for QTZ_ServerEngine 1.13
 *
 *
...
#include "stralloc.h"
	...
	static void add_justified(char *str, int slen, pad_info_t *pad, int fs, format_info finfo, short int trailing);
	 ...
	 * freed later.
	 */
	void sprintf_error(int which, char *premade)
	{
		char lbuf[2048];
		char *err;
		...
		switch (which)
		{
			case ERR_BUFF_OVERFLOW:
				err = "QTZ_ServerEngine:BUFF_SIZE overflowed...";
				break;
			case ERR_TO_FEW_ARGS:
				err = "QTZ_ServerEngine:More arguments specified than passed.";
				break;
			case ERR_INVALID_STAR:
				err = "QTZ_ServerEngine:Incorrect argument type to *.";
				break;
			case ERR_PRES_EXPECTED:
				err = "QTZ_ServerEngine:Expected presision not found.";
				break;
			case ERR_INVALID_FORMAT_STR:
				err = "QTZ_ServerEngine:Error in format string.";
				break;
			case ERR_INCORRECT_ARG_S:
				err = "QTZ_ServerEngine:Incorrect argument to type %%s.";
				break;
			case ERR_CST_REQUIRES_FS:
				err = "QTZ_ServerEngine:Column/table mode requires a field size.";
				break;
			case ERR_BAD_INT_TYPE:
				err = "QTZ_ServerEngine:!feature - bad integer type!";
				break;
			case ERR_UNDEFINED_TYPE:
				err = "QTZ_ServerEngine:!feature - undefined type!";
				break;
			case ERR_QUOTE_EXPECTED:
				err = "QTZ_ServerEngine:Quote expected in format string.";
				break;
			case ERR_UNEXPECTED_EOS:
				err = "QTZ_ServerEngine:Unexpected end of format string.";
				break;
			case ERR_NULL_PS:
				err = "QTZ_ServerEngine:Null pad string specified.";
				break;
			case ERR_ARRAY_EXPECTED:
				err = "QTZ_ServerEngine:Array expected.";
				break;
			case ERR_RECOVERY_ONLY:
				err = premade;
				break;
			default:
				err = "QTZ_ServerEngine:undefined error in (s)printf!\n";
				break;
		}
	...
	static void numadd(outbuffer_t *outbuf, int num)
	{
		...
		if ( (space = outbuf_extend(outbuf, num_l) ) )
		{
			...
			while (chop--)
			{
			 ...
			 /* lose that last digits that got chopped */
	 ...
	 * Scary number of parameters for a recursive fun.
	 */
			...
			case T_CLASS:
				{
					...
					numadd(outbuf, n);
					outbuf_add(outbuf, n == 1 ? (char *)" element\n" : (char *)" elements\n");
			...
			case T_FUNCTION:
				{
					...
					switch (obj->u.fp->hdr.type) {
						case FP_LOCAL | FP_NOT_BINDABLE: outbuf_add(outbuf, function_name(obj->u.fp->hdr.owner->prog, obj->u.fp->f.local.index) );
						break;
					case FP_SIMUL:
			...
			case T_OBJECT:
				{
					...
					if (!max_eval_error && !too_deep_error) {
						temp = apply(APPLY_OBJECT_NAME, obj->u.ob, 0, ORIGIN_DRIVER);
						if (temp && temp != (svalue_t*) - 1 && (temp->type == T_STRING) ) {
							outbuf_addv(outbuf, "['%s']", temp->u.string);
						}
					}
					break;
				}
			default:
				outbuf_add(outbuf, "QTZ_ServerEngine:ERROR SVALUE!");
	...
	INLINE_STATIC void add_nstr(char *str, int len)
	{
		if (outbuf_extend(&obuff, len) != len)
		{
			ERROR(ERR_BUFF_OVERFLOW);
	...
	static int add_table(cst **table)
	{
		...
		for (i = 0; i < tab->nocols && (tab_di = tab_d[i].cur); i++)
		{
			...
			for (done = 0; done != end && tab_di[done] != '\n'; done++)
				;
			add_justified(tab_di, (done > tab->size ? tab->size: done), tab->pad, tab->size, tab->info, tab->pad || (i < tab->nocols - 1) || tab->next);
			if (done >= end - 1)
			{
				tab_di = 0;
			}
	 ...
	 * this fun is called again, or if it's going to be modified (esp.
	...
	char *string_print_formatted(char *format_str, int argc, svalue_t *argv)
	{
		...
		if (guard) {
			guard = 0;
			error("QTZ_ServerEngine:Illegal to use sprintf() within the object_name() master call.\n");
		}
				...
				for (fpos++; !(finfo &INFO_T); fpos++) {
					...
					if ( ( (format_str[fpos] >= '0') && (format_str[fpos] <= '9') ) || (format_str[fpos] == '*') ) {
						if (pres ==  - 1) {
							...
							if ( (format_str[fpos] == '0') && ( ( (format_str[fpos + 1] >= '1') && (format_str[fpos + 1] <= '9') ) || (format_str[fpos + 1] == '*') ) )	{
								pad.what = "0";
								pad.len = 1;
							} else {
								if (format_str[fpos] == '*') {
						...
						if ( (finfo &INFO_COLS) || (finfo &INFO_TABLE) ) {
							...
							if (finfo &INFO_COLS) {
								...
								*temp = ALLOCATE(cst, TAG_TEMPORARY, "string_print: 3");
								...
								tmp = ( (format_str[fpos] != '\n') && (format_str[fpos] != '\0') ) || ( (finfo &INFO_ARRAY) && (nelemno < (argv + cur_arg)->u.arr->size) );
								...
								if (pres == 1) {
									...
									for (; i <= (unsigned)pres; i++) {
										(*temp)->d.tab[i].start = ++p1;
									}
								}
								for (i = 0; i < (unsigned)pres; i++) {
							...
							add_justified(carg->u.string, slen, &pad, fs, finfo, ( ( (format_str[fpos] != '\n') && (format_str[fpos] != '\0') ) || ( (finfo &INFO_ARRAY) && (nelemno < (argv + cur_arg)->u.arr->size) ) ) ||
 					carg->u.string[slen - 1] != '\n');
						...
						*cheat = '%';
						...
						if ( (cheat[i - 1] == 'f' && carg->type != T_REAL) || (cheat[i - 1] != 'f' && carg->type != T_NUMBER) )	{
								...
								sprintf(buff, "QTZ_ServerEngine:Incorrect argument type to %%%c. (arg: %u)\n", cheat[i - 1], cur_arg);
								fprintf(stderr, "Program /%s File: %s: %s", current_prog->name, get_line_number_if_any(), buff);
								debug_message("%s", buff);
								if (current_object)
								{
									debug_message("QTZ_ServerEngine: /%s, object: %s, file: %s\n", current_prog ? current_prog->name: "", current_object->name, get_line_number_if_any() );
								}
								ERROR(ERR_RECOVERY_ONLY);
							#else 
								error("QTZ_ServerEngine:Incorrect argument type to %%%c.\n", cheat[i - 1]);
						...
						}
						{
							...
							add_justified(temp, tmpl, &pad, fs, finfo, ( ( (format_str[fpos] != '\n') && (format_str[fpos] != '\0') ) || ( (finfo &INFO_ARRAY) && (nelemno < (argv + cur_arg)->u.arr->size) ) ) );
						}
					} else { /* type not found */
	...
	//return the len that fill the buf
	int b_vsnprintf(char *dest_str, int dest_size, char *fmt_str, int argc, svalue_t *argv)
	{
		char *buf = dest_str;
		char *fmt = fmt_str;
		int len = dest_size;
		...
		int i, fill = 0; //bytes have been filled, must <= len

		bzero(dest_str, dest_size);
		if (!argv)
		{
			return 0;
		}

		if (argv->type == T_ARRAY)
		{
			is_array_arg = 1;
			vec = argv->u.arr;
			argc = vec->size;
		...
		{
			int fill_len = 0;
			carg = is_array_arg ? vec->item + i: argv + i;
			...
			while (*fmt >= '0' &&  *fmt <= '9')
			{
				fill_len = fill_len * 10+ *fmt - '0';
				fmt++;
			}

			if (!fill_len)
			{
				if (i != argc - 1 ||  *fmt != 's' || carg->type != T_STRING)
				{
					ERROR(ERR_INVALID_FORMAT_STR);
				}

				fill_len = SVALUE_STRLEN(carg);
				if (fill + fill_len > len)
				{
					fill_len = len - fill; // no more than len
				}

			}
			else if (fill + fill_len >= len)
			{
				ERROR(ERR_BUFF_OVERFLOW);
			}
			...
			switch (*fmt++)
			{
				...
				case 'b':
					...
					#define FILL_BUF(t) *((t *)(buf+fill)) = (t) carg->u.number;
				...
				case 'c':
					//c will not add \0 to the end
					if (carg->type != T_STRING)
					{
						ERROR(ERR_INCORRECT_ARG_S);
					}
					strncpy(buf + fill, carg->u.string, fill_len);
					break;

				case 's':
					//s will add \0 to the end
					if (carg->type != T_STRING)
					{
						ERROR(ERR_INCORRECT_ARG_S);
					}
					...
					memcpy(buf + fill, carg->u.string, sLen);
					buf[fill + fill_len] = '\0';
			default:
					ERROR(ERR_INVALID_FORMAT_STR);
					break;
