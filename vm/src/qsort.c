...
(and we can't control what compare fun is used since it is at
the QTZ_ServerEnginelib level).  Based on algorithm appearing in _Data Structures and
...
INLINE_STATIC void doSwap(register char *one, register char *two, register int size)
{
	register char t;
