 ...
 * by the QTZ_ServerEngine driver.
 */
...
/*
static void *array_iter_head(fs_iterator *iter, array_t *arr);
static void *array_iter_next(fs_iterator *iter, array_t *arr);
static void *array_iter_info(fs_iterator *iter, array_t *arr);
static fs_iterator *array_iter_erase(fs_iterator *iter, array_t *arr);
*/
...
{
	1,  /* Ref count, which will ensure that it will // unsigned short ref;
	 * never be deallocated */
	#ifdef DEBUG
		1,  /* extra ref */
	#endif 
	0,  // dirty_manage_t * dirty_mng;      // 脏数据管理
	0,  /* size */
	/*
	array_iter_head,
	array_iter_next,
...
INLINE_STATIC array_t *ALLOC_ARRAY(int nelem)
{
	array_t * tmp = (array_t *)DXALLOC(sizeof (array_t) + sizeof(svalue_t) * (nelem - 1), TAG_ARRAY, "ALLOC_ARRAY");
	tmp->dirty_mng = NULL;
	//array_iter_set(tmp);
	return tmp;
}

INLINE_STATIC array_t *RESIZE_ARRAY(array_t *vec, int nelem) 
{
	array_t *tmp = (array_t *)DREALLOC(vec, sizeof (array_t) + sizeof(svalue_t) * (nelem - 1), TAG_ARRAY, "RESIZE_ARRAY");
	tmp->dirty_mng = NULL;
	//array_iter_set(tmp);
	return tmp;
}
...
array_t *explode_string(char *str, int slen, char *del, int len)
{
	...
	if (len == 1)
	{
		...
		for (p = str, beg = str, num = 0;  *p && (num < limit);)
		{
			if (*p == delimeter)
			{
				...
				DEBUG_CHECK(num >= ret->size, "Index out of bounds !\n");
				sz = p - beg;
	...
	for (p = str, beg = str, num = 0;  *p && (num < limit);)
	{
		if (strncmp(p, del, len) == 0)
		{
			...
			{
				fatal("In explode index is out of bounds !\n");
			}
 ...
 */
	...
	void filter_array(svalue_t *arg, int num_arg)
	{
		...
		else
		{
			...
			process_efun_callback(1, &ftc, F_FILTER_ARRAY);
...
These routines takes an array of objects and calls the fun 'func'
...
int sameval(svalue_t *arg1, svalue_t *arg2)
{
	...
	DEBUG_CHECK(!arg1 || !arg2, "error:Null pointer passed to sameval.\n");
...
//-------------------------------------------------------------------------
	...
	void unique_array_error_handler(void)
	{
		...
		for (i = 0; i < size; i++)
		{
			...
			if (sv && !sameval(sv, skipval) )
			{
				...
				while (uptr)
				{
					...
					{
						uptr->indices = RESIZE(uptr->indices, uptr->count + 1, int, TAG_TEMPORARY, "f_unique_array:2");
...
array_t *add_array(array_t *p, array_t *r)
{
	...
	{
		error("result of array addition is too greate.\n");
	}
	...
	if (r->ref == 1)
	{
		...
		#ifdef ARRAY_STATS
			num_arrays--;
			total_array_size -= sizeof(array_t) + sizeof(svalue_t)*(r->size - 1);
		#endif 
		FREE( (char*)r);
	}
	else
 ...
 */
	...
	void map_array(svalue_t *arg, int num_arg)
	{
		...
		else
		{
			...
			process_efun_callback(1, &ftc, F_MAP_ARRAY);
	...
	void map_string(svalue_t *arg, int num_arg)
	{
		...
		for (p = arr;  *p; p++)
		{
			...
			/* no fun or illegal return value is unaltered.
	...
	{
		quickSort( (char*)inlist->item, inlist->size, sizeof(inlist->item), (dir < 0) ? (int (*)(void *, void *))builtin_sort_array_cmp_rev : (int (*)(void *, void *))builtin_sort_array_cmp_fwd);
					...
					default:
						{
							...
							error("cannot handle the arrays of this type!");
						}
					}
				}
	...
	INLINE_STATIC int builtin_sort_array_cmp_rev(svalue_t *p1, svalue_t *p2)
	{
		...
		error("cannot handle such arrays! \n");
		return 0;
	}

	INLINE_STATIC int sort_array_cmp(svalue_t *p1, svalue_t *p2)
	{
		svalue_t *d;
	...
	void f_sort_array(void)
	{
		...
		switch (arg[1].type)
		{
			...
			case T_STRING:
				{
					/*
					 * We use a global to communicate with the comparison fun,
					...
					tmp = copy_array(tmp);
					quickSort( (char*)tmp->item, tmp->size, sizeof(tmp->item), (int (*)(void *, void *))sort_array_cmp);
...
INLINE_STATIC svalue_t *alist_sort(array_t *inlist)
{
	...
	for (j = 0; j < size; j++)
	{
		...
		for (curix = 0;;)
		{
			...
			if (child2 < size && sv_tab[child2].type != T_INVALID && (sv_tab[child1].type == T_INVALID || alist_cmp(sv_tab + child1, sv_tab + child2) > 0) )
			{
				child1 = child2;
			}
...
array_t *intersect_array(array_t *a1, array_t *a2)
{
	...
	for (j = 0; j < a2s; j++)
	{
		...
		for (curix = 0;;)
		{
			...
			if (child2 < a2s && sv_tab[child2].type != T_INVALID && (sv_tab[child1].type == T_INVALID || alist_cmp(sv_tab + child1, sv_tab + child2) > 0) )
			{
				child1 = child2;
			}
...
int match_single_regexp(char *str, char *pattern)
{
	#ifndef USE_POSIX_REGEXP
		struct regexp *reg;
		int ret;

		regexp_user = EFUN_REGEXP;
		reg = _regcomp( (unsigned char*)pattern, 0);
		if (!reg)
		{
			error(regexp_error);
		}
		ret = _regexec(reg, str);
		FREE( (char*)reg);
		return ret;
	#else 
		//编译过的正则表达式
		...
		//错误代码
		int err;
		char errbuf1[ERR_STR_LEN];
		char errbuf2[ERR_STR_LEN];
		regmatch_t subs[SUBLEN];

		err = regcomp(&reg, pattern, REG_EXTENDED | REG_ICASE);
		if (err)
		{
			//有错误
			regerror(err, &reg, errbuf1, sizeof(errbuf1) );
		...
		err = regexec(&reg, str, (size_t)SUBLEN, subs, 0);
		...
		{
			regerror(err, &reg, errbuf1, sizeof(errbuf1) );
			snprintf(errbuf2, ERR_STR_LEN, "QTZ_ServerEngine:reg exp find error %s\n", errbuf1);
			error(errbuf2);
			regfree(&reg);
			return 0;
		}
...
array_t *match_regexp(array_t *v, char *pattern, int flag)
{
	#ifndef USE_POSIX_REGEXP
		struct regexp *reg;
		char *res;
		...
		while (size--)
		{
			if (!( (--sv1)->type == T_STRING) || (_regexec(reg, sv1->u.string) != match) )
			{
				res[size] = 0;
			} 
			else
		...
		regex_t reg;
		//数组大小,匹配数量,错误代码
		int size, num_match, err, match = !(flag &2);
		char *res;
		char errbuf1[ERR_STR_LEN];
		char errbuf2[ERR_STR_LEN];
		regmatch_t subs[SUBLEN];
		...
		if (err)
		{
			//有错误
			regerror(err, &reg, errbuf1, sizeof(errbuf1) );
			snprintf(errbuf2, ERR_STR_LEN, "QTZ_ServerEngine:reg exp comile error |%s|%d|\n", pattern, err);
			error(errbuf2);
			return allocate_empty_array(0);
		}
		//初始化搜索标记数组大小
		...
		while (size--)
		{
			...
			{
				err = regexec(&reg, sv1->u.string, (size_t)SUBLEN, subs, 0);
				if ( (err == REG_NOMATCH) == match)
				{
					res[size] = 0;
				}
				else
...
array_t *children(char *str)
{
	...
	if (!(tmp_children = (object_t **)DMALLOC(sizeof(object_t*)*(t_sz = 50), TAG_TEMPORARY, "children: tmp_children") ) )
	{
		return  &the_null_array;
	}
	 /* unable to malloc enough temp space */

	for (i = 0, ob = obj_list; ob; ob = ob->next_all)
	{
		ol = strlen(ob->name);
		if ( ( (ol == sl) || ( (ol > sl) && (ob->name[sl] == '#') ) ) && !strncmp(tmpbuf, ob->name, sl) )
		{
			...
			tmp_children[i] = ob;
			if ( (++i == t_sz) && (!(tmp_children = RESIZE(tmp_children, (t_sz += 50), object_t *, TAG_TEMPORARY, "children: tmp_children: realloc") ) ) )
			{
...
//-------------------------------------------------------------------------
	...
	void f_objects(void)
	{
		...
		if (!(tmp = (object_t **)new_string( (t_sz = 1000) *sizeof(object_t*), "TMP: objects: tmp") ) )
	 ...
	 *
	 */
	array_t *reg_assoc(char *str, array_t *pat, array_t *tok, svalue_t *def)
	{
		int i, size;
		char *tmp;
		array_t *ret;
		...
		if (size)
		{
			...
			for (i = 0; i < size; i++)
			{
				if (!(rgpp[i] = _regcomp( (unsigned char*)pat->item[i].u.string, 0) ) )
				{
					while (i--)
					{
						FREE( (char*)rgpp[i]);
...
// iterator op
inline static int _iterator_op_check(fs_iterator *iter, array_t *arr)
{
	if (!iter) return 0;

	if (!arr) {
		FS_SET_ITERATOR_NULL(iter);
		return 0;
	}
...
/*static void *array_iter_head(fs_iterator *iter, array_t *arr)
{
	if (!_iterator_op_check(iter, arr)) {
		return NULL;
	}

	svalue_t *elt = arr->item;
...
void array_iter_set(array_t *arr)
{
	if (!arr) return;

	arr->iter_head = array_iter_head;
	arr->iter_next = array_iter_next;
	arr->iter_info = array_iter_info;
	arr->iter_erase = array_iter_erase;
}

void array_iter_unset(array_t *arr)
{
	if (!arr) return;
