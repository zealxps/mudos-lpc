...
INLINE int growMap(mapping_t *m)
{
    ...
    /*
    // 不考虑都又MAX_MAPPING_SIZE控制
	if (newsize > MAX_TABLE_SIZE)
	{
	    debug_message("Mapping exceeded maximum table size of %d.\n", MAX_TABLE_SIZE);
		return 0;
	}
    */
...
/*
mapTraverse: iterate over the mapping, calling fun 'func(elt, extra)'
	...
	num_mappings--;
	{
		...
		do
		{
			for (elt = a[j]; elt; elt = nelt)
			{
				...
				if(val->type == T_MAPPING && m->dirty_mng && val->u.refed->ref >=2) {
					debug_message("[%d] dirty_mng:%s.\n", time(NULL), string_print_formatted("%O", 1, val));
#ifdef 	DEVELOP_DIRTY_ASSERT
					assert(NULL) ; 
#endif
				}

				free_svalue(val, "free_mapping");
...
// #ifdef DEVELOP_MODE
	...
	switch(type) {
		...
		case T_STRING:
			break;
		default:
			/* debug_message("error mapping key type:%0x.\n",type); */
			/* assert(NULL); */
            throw_error();
			// fatal("error mapping key type:%0x.\n",type);
            return 0;
	}
// #endif
...
INLINE svalue_t *find_for_insert(mapping_t *m, svalue_t *lv, int doTheFree)
{
	...
	if (++m->count > MAX_MAPPING_SIZE)
	{
        ...
        check_mapping_too_large(m->count);
    }
...
void map_mapping(svalue_t *arg, int num_arg)
	{
		...
		process_efun_callback(1, &ftc, F_MAP_MAPPING);
	...
	void filter_mapping(svalue_t *arg, int num_arg)
	{
		...
		process_efun_callback(1, &ftc, F_FILTER_MAPPING);
...
inline static int _iterator_op_check(fs_iterator *iter, mapping_t *map)
{
	if (!iter) return 0;
...
// iterator op
void *mapping_iter_head(fs_iterator *iter, mapping_t *map)
{
	if (!_iterator_op_check(iter, map)) {
		return NULL;
	}

	mapping_node_t *elt = map->table[0];

	iter->ptr = (void *)elt;
	iter->data = (void *)elt->values;
	iter->cursor = 0;
	iter->u.pair.key = elt->values; 
	iter->u.pair.val = elt->values + 1;

	return iter;
}

void *mapping_iter_next(fs_iterator *iter, mapping_t *map)
{
	if (!_iterator_op_check(iter, map)) {
		return NULL;
	}

	mapping_node_t *elt = (mapping_node_t *)iter->ptr;
	mapping_node_t *nelt = elt->next;
	mapping_node_t *celt = NULL;
	if (!nelt) {
		// find next list
		int cursor = iter->cursor + 1;
		if (!map->table[cursor]) {
			// reach end
			FS_SET_ITERATOR_NULL(iter);
		}
		else {
			celt = map->table[cursor];
			iter->cursor = cursor;
		}
	}
	else {
	...
	if (celt) {
		iter->ptr = (void *)celt;
		iter->data = (void *)celt->values;
		iter->u.pair.key = celt->values; 
		iter->u.pair.val = celt->values + 1;
	}

	return iter;
}

void *mapping_iter_info(fs_iterator *iter, mapping_t *map)
{
	if (!_iterator_op_check(iter, map)) {
		return NULL;
	}
...
fs_iterator *mapping_iter_erase(fs_iterator *iter, mapping_t *map)
{
	if (!_iterator_op_check(iter, map)) {
		return NULL;
	}
	
	svalue_t *key = FS_ITERATOR_KEY(*iter);
	if (!node_find_in_mapping(map, key)) {
		// not found
		return iter;
	}
	...
	if (!nelt) {
		// try next list
		int ncursor = iter->cursor + 1;
		if (map->table[ncursor]) {
			// got it
			nelt = map->table[ncursor];
		}
	}
	...
	if (nelt) {
		...
		iter->ptr = (void *)nelt;
		iter->data = (void *)nelt->values;
		iter->u.pair.key = nelt->values;
		iter->u.pair.val = nelt->values + 1;
	}
	else {
		// end of list
		FS_SET_ITERATOR_NULL(iter);
...
void mapping_iter_set(mapping_t *map)
{
	...
	map->iter_head = mapping_iter_head;
	map->iter_next = mapping_iter_next;
	map->iter_info = mapping_iter_info;
	map->iter_erase = mapping_iter_erase;
	*/
}
