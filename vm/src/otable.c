...
/*
 * Object hash fun, ripped off from stralloc.c.
 */
...
void enter_object_hash(object_t *ob)
{
	...
	if (s != master_ob && s != simul_efun_ob) {
		DEBUG_CHECK1(s && s != ob, "OS debug: Duplicate object \"/%s\" in object hash table", ob->name);
	}
#endif 
...
void remove_object_hash(object_t *ob)
{
	...
	DEBUG_CHECK1(s != ob, "OS debug: Remove object \"/%s\": found a different object!", ob->name);
...
int show_otable_status(outbuffer_t *out, int verbose)
{
	...
	if (verbose == 1)
	{
		outbuf_add(out, "MudOS status: Object name hash table status:\n");
		outbuf_add(out, "------------------------------\n");
	...
	starts = (int)OTABLE_SIZE *sizeof(object_t*) + objs_in_table *sizeof(object_t);

	if (!verbose)
	{
		outbuf_addv(out, "MudOS Error: Obj table overhead:\t\t%8d %8d\n", OTABLE_SIZE *sizeof(object_t*), starts);
	}
	return starts;
}
