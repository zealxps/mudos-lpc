...
typedef int (*uar_func_t)(void *);

static void sprout(tree **, char *, int *, compare_func_t, uar_func_t);
static int delete_ (tree **, compare_func_t, char *, uar_func_t, int *, int*);
static void del(tree **, int *, tree **, uar_func_t, int*);
...
void tree_add(tree **ppr_tree, int (*pfi_compare)(), char *pc_user, int (*pfi_delete_)())
{
	int i_balance = 0;
	sprout(ppr_tree, pc_user, &i_balance, (compare_func_t)pfi_compare, (uar_func_t)pfi_delete_);
	return ;
}
...
static void sprout(tree **ppr, char *pc_data, int *pi_balance, int (*pfi_compare)(void *, void *), int (*pfi_delete_)(void *))
{
	tree *p1,  *p2;
	int cmp;
	...
	if (cmp < 0)
	{
		sprout(&(*ppr)->tree_l, pc_data, pi_balance, pfi_compare, pfi_delete_);
		if (*pi_balance)
		{
	...
	if (cmp > 0)
	{
		sprout(&(*ppr)->tree_r, pc_data, pi_balance, pfi_compare, pfi_delete_);
		if (*pi_balance)
		{
...
int tree_delete_(tree **ppr_p, int (*pfi_compare)(),char *pc_user, int (*pfi_uar)())
{
	int i_balance = 0, i_uar_called = 0;

	return delete_ (ppr_p, (compare_func_t)pfi_compare, pc_user, (uar_func_t)pfi_uar,  &i_balance, &i_uar_called);
}
...
static int delete_ (tree **ppr_p, int (*pfi_compare)(void *, void *), char *pc_user,
					int (*pfi_uar)(void *), int *pi_balance, int *pi_uar_called)
{
	...
	if (i_comp > 0)
	{
		i_ret = delete_ (&(*ppr_p)->tree_l, pfi_compare, pc_user, pfi_uar, pi_balance, pi_uar_called);
		if (*pi_balance)
		{
	...
	{
		i_ret = delete_ (&(*ppr_p)->tree_r, pfi_compare, pc_user, pfi_uar, pi_balance, pi_uar_called);
		if (*pi_balance)
		{
	...
	else
	{
		...
		{
			del(&pr_q->tree_l, pi_balance, &pr_q, (uar_func_t)pfi_uar, pi_uar_called);
			if (*pi_balance)
			{
				balanceL(ppr_p, pi_balance);
			}
