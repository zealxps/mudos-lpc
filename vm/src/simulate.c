...
object_t *current_object; /* The object interpreting a fun. */
 ...
 *
 */
object_t *int_load_object(const char *lname, int reload)
{
	int f;
	program_t *prog;
	...
	if (inherit_file) {
		 ...
		 * create fun. Without this check, that would crash the driver.
		 * -Beek
		 */
	...
	if (!reload) {
		safe_apply("on_loadin", ob, 0, ORIGIN_DRIVER);
	}
...
object_t *reload_object(const char *str)
{
	object_t *ob = find_object2(str);
	if (ob) {
		destruct_object(ob);
	}
...
void shutdown_os(int exit_code)
{
