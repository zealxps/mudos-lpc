...
error("Too deep (%d) for save_object\n",\
...
INLINE_STATIC int restore_internal_size(char **str, int is_mapping, int depth)
{
...
INLINE_STATIC int restore_size(char **str, int is_mapping)
{
	register char *cp =  *str;
	int size = 0;
	char c, delim, index = 0;
...
INLINE_STATIC int restore_interior_string(char **val, svalue_t *sv)
{
			...
			case '\\':
				{
					...
					if ( (*new_++ =  *cp++) )
					{
						...
						*new_ = '\0';
						*val = cp;
						sv->u.string = new_string(len = (new_ - start), "restore_string");
...
static int restore_mapping(char **str, svalue_t *sv)
{
	...
	value_numeral_error: free_svalue(&key, "restore_mapping: numeral value error");
	key_numeral_error: add_map_stats(m, count);
	free_mapping(m);
	return ROB_NUMERAL_ERROR;
	generic_value_error: free_svalue(&key, "restore_mapping: generic value error");
	generic_key_error: add_map_stats(m, count);
	free_mapping(m);
	return ROB_MAPPING_ERROR;
	value_error: free_svalue(&key, "restore_mapping: value error");
	key_error: add_map_stats(m, count);
	free_mapping(m);
	return err;
}
...
INLINE_STATIC int restore_class(char **str, svalue_t *ret)
{
	int size;
	char c;
	array_t *v;
	...
	*str = cp;
	...
	return 0;
	/* something went wrong */
	numeral_error: err = ROB_NUMERAL_ERROR;
	goto error;
	generic_error: err = ROB_CLASS_ERROR;
	error: free_class(v);
	return err;
}
...
INLINE_STATIC int restore_array(char **str, svalue_t *ret)
{
	int size;
	char c;
	array_t *v;
	...
	*str = cp;
	...
	numeral_error: err = ROB_NUMERAL_ERROR;
	goto error;
	generic_error: err = ROB_ARRAY_ERROR;
	error: free_array(v);
	return err;
}
...
INLINE int restore_string(char *val, svalue_t *sv)
{
	register char *cp = val;
	char *start = cp;
	char c;
			...
			case '\\':
				{
					...
					if ( (*new_++ =  *cp++) )
					{
						...
						*new_ = '\0';
						sv->u.string = new_string(new_ - start, "restore_string");
						strcpy(sv->u.string, start);
						sv->type = T_STRING;
						sv->subtype = STRING_MALLOC;
						return 0;
...
INLINE int restore_svalue(char *cp, svalue_t *v)
{
	int ret;
	char c;
...
an error */
INLINE int safe_restore_svalue(char *cp, svalue_t *v)
{
	int ret;
	svalue_t val;
	char c;
...
void restore_object_from_buff(object_t *ob, char *theBuff, int noclear)
{
	...
	while ( (buff = nextBuff) &&  *buff)
	{
		...
		if (buff[0] == '#')
		 /* ignore 'comments' in savefiles */
		{
			continue;
		}
		space = strchr(buff, ' ');
		if (!space || ( (unsigned) (space - buff) >= sizeof(var) ) )
		{
			FREE(theBuff);
		...
		if (rc &ROB_ERROR)
		{
			...
			{
				error("QTZ_ServerEngine:Bad format while restoring %s.\n", var);
			}
 ...
 * The routine checks with the fun "valid_write()" in /obj/master.c
...
static int save_object_recurse(program_t *prog, svalue_t **svp, int type, int save_zeros, FILE *f)
{
	...
	for (i = 0; i < prog->num_inherited; i++)
	{
		if (!save_object_recurse(prog->inherit[i].prog, svp, prog->inherit[i].type_mod | type, save_zeros, f) )
		{
			return 0;
		}
	}
	...
	for (i = 0; i < prog->num_variables_defined; i++)
	{
		...
		*new_str = '\0';
		...
		if (save_zeros || new_str[0] != '0' || new_str[1] != 0)
		 /* Armidale */
...
{
	char *name;
	static char tmp_name[256], bak_name[256];
	int len;
	FILE *f;
	int success;
	svalue_t *v;
	...
	{
		error("QTZ_ServerEngine:Denied write permission in fun.\n");
	}
	...
	if (!(f = fopen(tmp_name, "w") ) || fprintf(f, "#/%s\n", ob->prog->name) < 0)
	{
		...
		error("QTZ_ServerEngine:Could not open /%s for a save.\n", tmp_name);
	}
	...
	if (fclose(f) < 0)
	{
		...
		debug_perror("QTZ_ServerEngine:save_object", file);
		success = 0;
	}
	...
	if (!success)
	{
		...
		debug_message("Failed to save file. Disk may be full.\n");
		unlink(tmp_name);
	}
	else
	{
		sprintf(bak_name, "%.250s.bak", file);
		unlink(bak_name);
		link(file, bak_name);
		#ifdef WIN32
		...
		if (rename(tmp_name, file) < 0)
		{
			...
			debug_perror("QTZ_ServerEngine:save object", file);

			debug_message("QTZ_ServerEngine:Failed to rename /%s to /%s\n", tmp_name, file);

			debug_message("QTZ_ServerEngine:Failed to save object!\n");
			unlink(tmp_name);
		}
	}
...
/*
int save_log(char *filename, char *info)
{
...
char *GetMidStr(char *str1, const char *str2, const char *str3)
{
	char *res;
	...
	res = strstr(temp, str2);
	if (res)
	{
		res = res + strlen(str2);
		strcpy(temp, res);
		res = strstr(temp, str3);
		if (res)
		{
			temp[res - temp] = '\0';
			strcpy(str1, temp);
...
int restore_object(object_t *ob, char *file, int noclear)
{
	...
	{
		debug_message("QTZ_ServerEngine:Object /%s restored from /%s.\n", ob->name, file);
	}
#endif 
	FREE(theBuff);
	return 1;
...
void restore_variable(svalue_t *var, char *str)
{
	...
	if (rc &ROB_ERROR)
	{
		*var = const0; /* clean up */
		...
		{
			error("QTZ_ServerEngine:Wrong general format to restore object.\n");
		}
		else if (rc &ROB_NUMERAL_ERROR)

		{
			error("QTZ_ServerEngine:Wrong format to restore object.\n");
		}
		else if (rc &ROB_ARRAY_ERROR)
...
void dealloc_object(object_t *ob, char *from)
{
	...
	{
		debug_message("QTZ_ServerEngine:free object: /%s.\n", ob->name);
	}
#endif 
	if (!(ob->flags &O_DESTRUCTED) )
	{
		...
		fatal("QTZ_ServerEngine: \n FATAL: Object 0x%x /%s ref count 0, but not destructed (from %s).\n", ob, ob->name, from);
	}

	DEBUG_CHECK(ob->interactive, "Can't free an interactive object.\n");
	/*
	...
	if (ob->name)
	{
		...
		DEBUG_CHECK1(lookup_object_hash(ob->name) == ob, "QTZ_ServerEngine:That object is free /%s but name still in name table", ob->name);
		FREE(ob->name);
		ob->name = 0;
	}
	tot_alloc_object--;
...
void free_object(object_t *ob, char *from)
{
	ob->ref--;
...
void reset_object(object_t *ob)
{
	...
	ob->next_reset = current_time + TIME_TO_RESET / 2+random_number(TIME_TO_RESET / 2);
...
void call_create(object_t *ob, int num_arg)
{
	...
	safe_apply(APPLY_CREATE, ob, num_arg, ORIGIN_DRIVER);
...
svalue_t *v_call_function(object_t *obj, const char *name, const char *fmt, va_list args)
{
	if (obj == NULL)
		return &const0u;
	...
	while (*fmt) {
		...
		case 's': {
			...
			break;
		case 'b': {
			buffer_t *arg = va_arg(args, buffer_t*);
			push_buffer(arg);
			argc++;
		}
			break;
		case 'm': {
			svalue_t *arg = va_arg(args, svalue_t *);
			push_svalue(arg);
			argc++;
		}
			break;
		case 'o': {
			...
			break;
		default:
			debug_message("Error Script Function Param\n");
			break;
		}
	...
	svalue_t *ret = safe_apply(name, obj, argc, ORIGIN_DRIVER);
	if(!ret) {
		ret = &const0u;
	}
...
svalue_t *call_script_func(const char *obj_name, const char *name,const char *fmt, ...)
{
	va_list args;
	object_t *object = NULL;

	object = find_object(obj_name);
	...
	eval_cost = max_cost;
	va_start(args, fmt);
	svalue_t *ret = v_call_function(object, name, fmt, args);
	va_end(args);
	return ret;
}

svalue_t *call_object_func(object_t *object,const char *name,const char *fmt, ...)
{
	va_list args;
...
{
	unsigned short type;
        int index = find_global_variable(object->prog, name, &type);
	return index == -1 ? &const0u : &object->variables[index];
}
