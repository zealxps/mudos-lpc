...
#ifdef DEBUG
		if (d_flag) {
			printf("QTZ_ServerEngine:Sub fun ref /%s: now %i\n", fp->f.functional.prog->name, fp->f.functional.prog->func_ref);
		}
#endif 
		if (fp->f.functional.prog->func_ref == 0 && fp->f.functional.prog->ref == 0) {
		...
		case T_NUMBER:
			{
				...
				{
					error("QTZ_ServerEngine:Division wrong\n");
				}
				sp->u.number = argp->u.number /= sp->u.number;
				sp->subtype = 0;
				break;
		...
		case T_REAL:
			...
			case T_NUMBER | T_REAL: 
			{
				...
				else
				{
					...
					{
						error("QTZ_ServerEngine:Division\n");
					}
		...
		default:
			{
				...
				{
					error("QTZ_ServerEngine:Bad type to /=\n");
				}
			}
	}
}
...
void f_ge()
{
	...
	switch ( (--sp)->type | i)
	{
		...
		case T_REAL:
			...
			put_number(i);
			break;
			case T_NUMBER | T_REAL: if (i == T_NUMBER)
			{
				sp->type = T_NUMBER;
				sp->u.number = sp->u.real >= (sp + 1)->u.number;
			}
...
void f_mod_eq()
{
	...
	{
		error("QTZ_ServerEngine:Modulo wrong\n");
	}
	sp->u.number = argp->u.number %= sp->u.number;
	sp->subtype = 0;
}
...
void f_range(int code)
{
	...
	{
		error("QTZ_ServerEngine:That needs a number.\n");
	}
	...
	switch (sp->type)
	{
		...
		default:
			...
			error("QTZ_ServerEngine:Bad argument to that operator.\n");
	}
}
		...
		default:
			{
				...
				{
					error("QTZ_ServerEngine:Arguments to -= .\n");
				}
			}
	}
}
...
void f_switch()
{
	...
	{
		0 *SWITCH_CASE_SIZE, 1 *SWITCH_CASE_SIZE, 3 *SWITCH_CASE_SIZE, 7 *SWITCH_CASE_SIZE, 15 *SWITCH_CASE_SIZE, 31 *SWITCH_CASE_SIZE, 63 *SWITCH_CASE_SIZE, 127 *SWITCH_CASE_SIZE, 255 *SWITCH_CASE_SIZE,
	511 *SWITCH_CASE_SIZE, 1023 *SWITCH_CASE_SIZE, 2047 *SWITCH_CASE_SIZE, 4095 *SWITCH_CASE_SIZE, 8191 *SWITCH_CASE_SIZE, 
	};
	...
	if (i >= 14)
	{
		...
		{
			fatal("QTZ_ServerEngine:switch table format don't support.\n");
		}
	}
...
void call_simul_efun(unsigned short index, int num_arg)
{
	...
	{
		error("Function is not a simul_efun.\n");
	}
}
...
funptr_t *make_efun_funp(int opcode, svalue_t *args)
{
	...
	fp = (funptr_t*)DXALLOC(sizeof(funptr_hdr_t) + sizeof(efun_ptr_t), TAG_FUNP, "make_efun_funp");
...
funptr_t *make_lfun_funp(int index, svalue_t *args)
{
	...
	fp = (funptr_t*)DXALLOC(sizeof(funptr_hdr_t) + sizeof(local_ptr_t), TAG_FUNP, "make_efun_funp");
	fp->hdr.owner = current_object;
	add_ref(current_object, "make_efun_funp");
...
funptr_t *make_simul_funp(int index, svalue_t *args)
{
	...
	fp = (funptr_t*)DXALLOC(sizeof(funptr_hdr_t) + sizeof(simul_ptr_t), TAG_FUNP, "make_efun_funp");
	fp->hdr.owner = current_object;
	add_ref(current_object, "make_efun_funp");
...
funptr_t *make_functional_currying(funptr_t *old, int merge_arg_num)
{
	funptr_t *fp;

	fp = (funptr_t*)DXALLOC(sizeof(funptr_hdr_t) + sizeof(functional_t), TAG_FUNP, "make_functional_currying");
	fp->hdr.owner = old->hdr.owner;
	add_ref(current_object, "make_functional_currying");
	fp->hdr.type = old->hdr.type;

	current_prog->func_ref++;

#ifdef DEBUG
	if (d_flag)	{
		printf("QTZ_ServerEngine:Add fun ref /%s: now %i\n", current_prog->name, current_prog->func_ref);
	}
#endif


	fp->f.functional.prog = old->f.functional.prog;
	fp->f.functional.offset = old->f.functional.offset;
	fp->f.functional.num_arg = old->f.functional.num_arg;
	fp->f.functional.num_local = old->f.functional.num_local;
	fp->f.functional.num_upvalue = old->f.functional.num_upvalue;
	fp->f.functional.fio = old->f.functional.fio;
	fp->f.functional.vio = old->f.functional.vio;

	fp->hdr.upvalues = old->hdr.upvalues;
	if (fp->hdr.upvalues) fp->hdr.upvalues->ref++;
	// todo

	pop_n_elems(merge_arg_num);
	/*
	fp->hdr.args = old->hdr.args;
	*/
	//pc += len;
	fp->hdr.ref = 1;
	return fp;
...
//
//-------------------------------------------------------------------------
funptr_t *make_functional_funp(short num_arg, short num_local, short len, svalue_t *upvalues, int flag)
{
	funptr_t *fp;

	fp = (funptr_t*)DXALLOC(sizeof(funptr_hdr_t) + sizeof(functional_t), TAG_FUNP, "make_functional_funp");
	fp->hdr.owner = current_object;
	...
	int num_upvalue = 0;
	if (upvalues && upvalues->type == T_ARRAY)
		num_upvalue = upvalues->u.arr->size;
#ifdef DEBUG
	if (d_flag)	{
		printf("QTZ_ServerEngine:Add fun ref /%s: now %i\n", current_prog->name, current_prog->func_ref);
		printf("num_arg=%d,num_local=%d,num_upvalue=%d\n", num_arg, num_local, num_upvalue);
	}
#endif
	...
	fp->f.functional.num_upvalue = num_upvalue;
	...
	if (upvalues && upvalues->type == T_ARRAY) {
		fp->hdr.upvalues = upvalues->u.arr;
		upvalues->u.arr->ref++;
#ifdef DEBUG
		printf("%s", string_print_formatted("%O", 1, upvalues));
#endif
	} else {
		fp->hdr.upvalues = 0;
	}
...
void f_function_constructor()
{
	...
	case FP_ANONYMOUS | FP_NOT_BINDABLE: {
		...
		fp = make_functional_funp(num_arg, locals, index, sp, kind &FP_NOT_BINDABLE);
		pop_stack();
		break;
	}
	default:

			fatal("Can't not make unknown type of fun pointer.\n");
	}
	push_refed_funp(fp);
}
...
void f_evaluate(void)
{
	...
	if (arg->type != T_FUNCTION) {
		...
		error("Bad type (function type required), when evaluate\n");
		return ;
	}
	if (current_object->flags &O_DESTRUCTED) {
		pop_n_elems(st_num_arg);
		push_undefined();
		error("Object destructed, when evaluate\n");
		return ;
	}
