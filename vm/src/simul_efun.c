...
/*
We now keep track of two mappings:
 *     name -> index       and     index -> fun
 * 
 * index->fun is used at runtime since it's very fast.  name->index
 * is used at compile time.  It's sorted so we can search it in O(log n)
 * as opposed to a linear search on the fun table.  Note that we
 * can't sort the fun table b/c then indices wouldn't be preserved
 * across updates.
 *
...
void init_simul_efun(const char *file)
{
	...
	if (!file || !file[0])
	{
		fprintf(stderr, "efun error: No simul_efun\n");
		return ;
	}
	if (!strip_name(file, buf, sizeof buf) )
	{
		error("OS error: Ilegal simul_efun file name '%s'\n", file);
	}
	...
	if (new_ob == 0)
	{
		fprintf(stderr, "OS error: The simul_efun file %s was not loaded.\n", buf);
		exit( - 1);
	}
