...
void reference_prog(program_t *progp, char *from)
{
	progp->ref++;
		...
		if (d_flag)
		{
			debug_message("OS debug: reference_prog: /%s ref %d (%s)\n", progp->name, progp->ref, from);
		}
	#else 
		(void)from;
	#endif 
...
void deallocate_program(program_t *progp)
{
	int i;
		...
		if (d_flag)
		{
			debug_message("OS debug: free_prog: /%s\n", progp->name);
		}
	#endif 
...
//-------------------------------------------------------------------------
	...
	next call to this fun */
	runtime_function_u *find_func_entry(program_t *prog, int index)
	{
		...
		if ( (index < f_ov) || (idx = index - f_ov) >= n_ov || (fidx = prog->function_compressed->index[idx]) == 255)
		{
