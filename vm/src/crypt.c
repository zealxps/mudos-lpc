/* Password hasher for the LPC crypt() fun.
 *
...
/* Table T constructed from a sine fun, mentioned in RFC, section 3.4.
...
/* This fun returns success, i.e. 0 on error. */
...
void getsalt(byte *to, CONST byte *from)
{
	...
	if (from)
	{
		 ...
		 * this fun. Well, suit yourself, we'll /get/ a salt
...
char *custom_crypt(CONST char *key, CONST char *salt, byte *rawout)
{
	...
	#define ADDBUFFER(b, l) if(used + (l) > (int)sizeof(buffer)) crunchbuffer(buffer, &used, (char *)(b), (l), sizeof(buffer)); else { memcpy(&(buffer[used]), (b), (l)); used += (l); }
...
unsigned char *md5_digest(unsigned char *buf, const char *input, int size)
{
	...
	struct md5_ctx ctx;
	md5_init_ctx(&ctx);
	md5_process_bytes((const unsigned char*)input, size, &ctx);
	md5_finish_ctx(&ctx, buf);
...
int hexdecode(const char *src, unsigned char *dest, int size)
{
	...
	for (i = 0; i < size; ++i)
	{
		...
		buf[2] = 0;
		dest[i] = (unsigned char)strtoul(buf, NULL, 16);
	}
	return 1;
}
