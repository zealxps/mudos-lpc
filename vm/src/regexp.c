 ...
 */
regexp *_regcomp(unsigned char *exp, int excompat) /* \( \) operators like in unix ex */
{
	register regexp *r;
	...
	exp2 = (short*)DXALLOC( (strlen( (char*)exp) + 1)*(sizeof(short[8]) / sizeof(char[8]) ), TAG_TEMPORARY, "regcomp: 1");
	for (scan = exp, dest = exp2; (c =  *scan++);)
	{
		switch (c)
		{
			...
			case '\\':
				...
				case 0:
					FREE(exp2);
					FAIL("OS error: Regular expression cannot end with '\\'.  Use \"\\\\\".\n");
					break;
				case '(':
				case ')':
				...
				case '}':
					FREE(exp2);
					FAIL("OS error: sorry, unimplemented operator\n");
				case 'b':
					*dest++ = '\b';
					break;
				case 't':
...
static char *regpiece(int *flagp)
{
	...
	{
		FAIL("OS error: *+ operand could be empty\n");
	}
		...
		case LSQBRAC:
			{
				...
				while (*regparse != '\0' &&  *regparse != RSQBRAC)
				{
					if (*regparse == '-')
					{
						...
						{
							class_  = (CHARBITS &*(regparse - 2) ) + 1;
							classend = (CHARBITS &*(regparse) );
							if (class_  > classend + 1)
							{
								FAIL("invalid [] range\n");
							}
							for (; class_  <= classend; class_ ++)
							{
								regc(class_ );
							}
							regparse++;
...
case OR_OP:
case RBRAC:
	FAIL("OS error: internal urp\n"); /* Supposed to be caught earlier. */
	break;
case ASTERIX:
	FAIL("OS error: * follows nothing\n");
	break;
case PLUSS:
	FAIL("OS error: + follows nothing\n");
	break;
...
default:
	{
			...
			regparse--;
			for (len = 0; regparse[len] && !(regparse[len] &SPECIAL) && regparse[len] != RSQBRAC; len++)
				;
			if (len <= 0)
			{
					FAIL("OS error: unexpected ]\n");
			}
			ender = *(regparse + len);
 ...
 */
int _regexec(register regexp *prog, register char *string)
{
		register char *s;
		...
		{
				regerror("OS error: NULL parameter\n");
				return (0);
		}
		/* Check validity of program. */
		if (UCHARAT(prog->program) != MAGIC)
		{
				regerror("OS error: corrupted program\n");
				return (0);
		}
...
static int regmatch(char *prog)
{
		...
		while (scan != (char*)NULL)
		{
				...
				switch (OP(scan) )
				{
						...
						case END:
							...
							break;
						default:
							regerror("OS error: memory corruption\n");
							return (0);
							break;
				}
		 ...
		 * terminating point.
		 */
		regerror("OS error: corrupted pointers\n");
		return (0);
}
...
static int regrepeat(char *p)
{
		...
		switch (OP(p) )
		{
				...
				default:
					...
					regerror("OS error: internal foulup\n");
	...
	static char *regprop(char *op)
	{
			...
			switch (OP(op) )
			{
					...
					case WORDEND:
						p = "WORDEND";
						break;
						case OPEN + 1: case OPEN + 2: case OPEN + 3: case OPEN + 4: case OPEN + 5: case OPEN + 6: case OPEN + 7: case OPEN + 8: case OPEN + 9: sprintf(buf + strlen(buf), "OPEN%d", OP(op) - OPEN);
						p = (char*)NULL;
						break;
						case CLOSE + 1: case CLOSE + 2: case CLOSE + 3: case CLOSE + 4: case CLOSE + 5: case CLOSE + 6: case CLOSE + 7: case CLOSE + 8: case CLOSE + 9: sprintf(buf + strlen(buf), "CLOSE%d", OP(op) -
	CLOSE);
						p = (char*)NULL;
						break;
					case STAR:
...
char *regsub(regexp *prog, char *source, char *dest, int n)
{
		...
		while ( (c =  *src++) != '\0')
		{
				...
				else if (prog->startp[no] != (char*)NULL && prog->endp[no] != (char*)NULL)
				{
						...
						{
								 /* amylaar */
								regerror("OS error: line too long\n");
								return NULL;
						}
						...
						{
								 /* strncpy hit NUL. */
								regerror("OS error: damaged match string\n");
								return NULL;
						}
				}
		}
