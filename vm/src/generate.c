...
#ifdef DEBUG
void dump_tree(parse_node_t *node);
#endif
...
static parse_node_t *optimize(parse_node_t *expr)
{
	...
	case NODE_ANON_FUNC:
		if (expr->l.expr) {
			optimize_expr_list(expr->l.expr);
		}
		break;
	case NODE_EFUN:
		optimize_expr_list(expr->r.expr);
...
void lpc_tree_form(parse_node_t *expr, parse_node_t *dest);
	...
	{
		CREATE_STRING(dest->v.expr, (char *)str);
	}
...
void optimizer_start_function(int n)
{
	if (n) {
...
void optimizer_end_function(void)
{
	int i;
	if (last_local_refs) {
...
//-------------------------------------------------------------------------
	...
	{
		printf("QTZ_ServerEngine:(lvalue_list ");
		while ( (expr = expr->r.expr) ) {
	...
	void dump_tree(parse_node_t *expr)
	{
		...
		switch (expr->kind) {
			...
			case NODE_BRANCH_LINK:
				...
				break;
			case NODE_CALL_2:
				printf("(%s %i %i %i ", instrs[expr->v.number].name, expr->l.number >> 16, expr->l.number &0xffff, (expr->r.expr ? expr->r.expr->kind: 0) );
				dump_expr_list(expr->r.expr);
				printf(")");
				break;
			case NODE_CALL_1:
				printf("(%s %i %i ", instrs[expr->v.number].name, expr->l.number, (expr->r.expr ? expr->r.expr->kind: 0) );
				dump_expr_list(expr->r.expr);
				printf(")");
				break;
			case NODE_CALL:
			...
			case NODE_CATCH:
				...
				break;
			case NODE_LVALUE_EFUN:
				printf("QTZ_ServerEngine:\n (lvalue_efun ");
				dump_tree(expr->l.expr);
				dump_lvalue_list(expr->r.expr);
				printf(")");
				break;
			case NODE_FUNCTION_CONSTRUCTOR:
				printf("(fun %i ", expr->v.number &0xff);
				if (expr->r.expr)
				{
					printf("(array ");
					dump_expr_list(expr->r.expr);
				...
				case FP_EFUN:
					...
					break;
				case FP_FUNCTIONAL:
					case FP_FUNCTIONAL | FP_NOT_BINDABLE: printf("(fp-functional %i ", expr->v.number >> 8);
					dump_tree(expr->l.expr);
					printf(")");
					break;
				}
