...
static sp_block_t scratch_head = 
{
	0, 0, 
	{
		0
...
{
	sp_block_t *next;
	sp_block_t *this_ = scratch_head.next;

	SDEBUG(printf("OS debug: scratch_destroy\n") );
...
char *scratch_copy(char *str)
{
	...
	SDEBUG2(printf("OS debug: scratch_copy(%s):", str) );
...
void scratch_free(char *ptr)
{
	...
	SDEBUG2(printf("OS debug: scratch_free(%s): ", ptr) );

	if (Ptr == scr_last)
	{
		SDEBUG2(printf("OS debug: last freed\n") );
		scratch_free_last();
	}
	else if (*(Ptr - 2) )
	{
		...
		SDEBUG(printf("QtzOS debug:block freed\n") );
		sbt = FIND_HDR(ptr);
		if (sbt->prev)
		{
	...
	{
		SDEBUG(printf("QtzOS debug: interior free\n") );
...
{
	sp_block_t *spt;
	SDEBUG(printf("OS debug: scratch_large_alloc(%i)\n", size) );
