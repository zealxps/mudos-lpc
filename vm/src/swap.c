...
static int swap_in(char **, long);
static int swap_out(char *, int, long*);
...
static int assert_swap_file()
{
	if (swap_file == NULL)
	{
			...
			sprintf(file_name_buf, "%s.%s.%d", SWAP_FILE, host, external_port[0].port);
 ...
 *  Seek the swap file
 */
static void swap_seek(long offset, int flag)
{
	int ret;
	...
	if (ret ==  - 1)
	{
		fatal("QTZ_ServerEngine:Couldn't seek the swap file, error %s, offset %d.\n", port_strerror(errno), offset);
	}
}
...
static void free_swap(int start, int length)
{
	...
	{
		DEBUG_CHECK(m->next, "QTZ_ServerEngine:extraneous free swap blocks!\n");
 ...
 */
static int swap_out(char *block, int size, long *locp)
{
	if (1)
	{
		return 0;
	...
	if (*locp ==  - 1)
	{
		 /* needs data written out */
		*locp = alloc_swap(size + sizeof size);
		swap_seek(*locp, 0);
		#ifdef SWAP_USE_FD
			if ( (write(swap_file, &size, sizeof size) != sizeof size) || write(swap_file, block, size) != size)
			{
		...
		#else 
			if (fwrite( (char*) &size, sizeof size, 1, swap_file) != 1 || fwrite(block, size, 1, swap_file) != 1)
			{
 ...
 */
static int swap_in(char **blockp, long loc)
{
	int size;
	...
	swap_seek(loc, 0);
		...
		{
			fatal("QTZ_ServerEngine:Couldn't read the swap file.\n");
		}
	...
	#else 
		/* find out size */
		if (fread( (char*) &size, sizeof size, 1, swap_file) == 0)
		{
			fatal("QTZ_ServerEngine:Couldn't read the swap file.\n");
		}
		*blockp = DXALLOC(size, TAG_SWAP, "swap_in");
		if (fread(*blockp, size, 1, swap_file) == 0)
		{
			fatal("QTZ_ServerEngine:Couldn't read the swap file.\n");
		}
	#endif 
...
int locate_out(program_t *prog)
{
	if (!prog)
	{
		...
		if (d_flag > 1)
		{
			debug_message("QTZ_ServerEngine:locate_out: %lX %lX %lX %lX %lX %lX %lX\n", prog->program, prog->function_table, prog->strings, prog->variable_table, prog->inherit, prog->argument_types, prog
 				->type_start);
		}
	#endif 
...
int locate_in(program_t *prog)
{
	...
	if (prog->type_start)
	{
		...
		if (d_flag > 1)
		{
			debug_message("QTZ_ServerEngine:locate_in: %lX %lX %lX %lX %lX %lX\n", prog->program, prog->function_table, prog->strings, prog->variable_table, prog->inherit, prog->argument_types, prog
 				->type_start);
		}
	#endif 
	return 1;
 ...
 *
 * marion - the swap seems to corrupt the fun table
 */
int swap(object_t *ob)
{
	...
	if (ob->flags &O_DESTRUCTED)
	{
		...
		{
			 /* marion */
			debug_message("QTZ_ServerEngine:Swap object /%s (ref %d)\n", ob->name, ob->ref);
		}
	#endif 
	if (ob->prog->line_info)
	{
	 ...
	 /* not always done before we get here */
	if ( (ob->flags &O_HEART_BEAT) || (ob->flags &O_CLONE) )
	{
		#ifdef DEBUG
			if (d_flag > 1)
			{
				debug_message("QTZ_ServerEngine:  object not swapped - heart beat or cloned.\n");
			}
		#endif 
		return 0;
	}
	if (ob->prog->ref > 1 || ob->interactive)
	{
			...
			if (d_flag > 1)
			{
				debug_message("QTZ_ServerEngine:  object not swapped - inherited or interactive.\n");
			}
		#endif 
		return 0;
	}
	if (ob->prog->func_ref > 0)
	{
			...
			if (d_flag > 1)
			{
				debug_message("QTZ_ServerEngine:  object not swapped - referenced by functions.\n");
			}
		#endif 
		return 0;
	}
	locate_out(ob->prog); /* relocate the internal pointers */
	if (swap_out( (char*)ob->prog, ob->prog->total_size, &ob->swap_num) )
	{
		num_swapped++;
...
{
	if (ob->swap_num ==  - 1) {
		fatal("QTZ_ServerEngine:Loading not swapped object.\n");
	}
#ifdef DEBUG
	if (d_flag > 1) {
		/* marion */
		debug_message("QTZ_ServerEngine:Unswap object /%s (ref %d)\n", ob->name, ob->ref);
	}
#endif 
	swap_in(&ob->prog->name, ob->swap_num);
 ...
 */
int swap_line_numbers(program_t *prog)
{
	int size;
	...
	if (!prog || !prog->line_info)
	{
		...
		if (d_flag > 1)
		{
			debug_message("QTZ_ServerEngine:Swap line numbers for /%s\n", prog->name);
		}
	#endif 
	size = prog->file_info[0];
	if (swap_out( (char*)prog->file_info, size, (long*) (void*) &prog->line_swap_index) )
	{
...
void load_line_numbers(program_t *prog)
{
	...
	if (prog->line_info)
	{
		...
		if (d_flag > 1)
		{
			debug_message("QTZ_ServerEngine:Unswap line numbers for /%s\n", prog->name);
		}
	#endif 
	size = swap_in( (char **) (void*) &prog->file_info, prog->line_swap_index);
 ...
 */
void remove_line_swap(program_t *prog)
{
	if (!prog->line_info)
	{
	...
	{
		free_swap(prog->line_swap_index, prog->file_info[0]);
	}
