 ...
 * Look at htable.c for the other code.  It uses the Hash() fun
...
#ifdef DEBUG
	static void checked(char *, char*);
#endif 

void dump_all_strings(FILE *f)
{
	int x; 
	for (x = 0; x < htable_size; x++)
	{
		block_t * blk = base_table[x], *tmp;
		if(!blk) continue; 
...
INLINE_STATIC block_t *sfindblock(const char *s, int h)
{
	block_t *curr,  *prev;
...
INLINE_STATIC block_t *alloc_new_string(const char *string, int h)
{
	block_t *b;
...
/*
ref_string: Fatal to call this fun on a string that isn't shared.
 */
...
char *ref_string(char *str)
{
	...
	b = BLOCK(str);
		...
		if (b != findblock(str) )
		{
			fatal("QTZ_ServerEngine in stralloc.c: called ref_string on non-shared string: %s.\n", str);
		}
	#endif /* defined(DEBUG) */
	if (REFS(b) )
	{
...
/*
 * fun called on free_string detected errors; things return checked(s).
 */
#ifdef DEBUG
	static void checked(char *s, char *str)
	{
		debug_message("%s (\"%s\")\n", s, str);
...
void free_string(char *str)
{
	...
	b = BLOCK(str);
	DEBUG_CHECK1(b != findblock(str), "QTZ_ServerEngine in stralloc.c: free_string called on non-shared string: %s.\n", str);
	...
	while ( (b =  *prev) )
	{
		...
		if (!b)
		{
			checked("QTZ_ServerEngine in stralloc.c free_string: not found in string table!", str);
			return ;
		}
	#endif 
...
void deallocate_string(char *str)
{
	...
	DEBUG_CHECK1(!b, "QTZ_ServerEngine in stralloc.c: deallocate_string called on non-shared string: %s.\n", str);
