...
/* This fun has strput() semantics; see comments in simulate.c */
char *get_two_types(char *where, char *end, int type1, int type2)
{
	where = strput(where, end, "( ");
	where = _get_type_name(where, end, type1);
	where = strput(where, end, "vs ");
	where = _get_type_name(where, end, type2);
	where = strput(where, end, ")");
...
int local_count_stack[256];
int local_count_stack_sp = 0;

void add_local_count(int val)
{
	current_local_count += val;
}
...
void push_local_count()
{
	local_count_stack[local_count_stack_sp++] = current_local_count;
	current_local_count = 0;
}

void pop_local_count()
{
	current_local_count = local_count_stack[--local_count_stack_sp];
}

//-------------------------------------------------------------------------
// upvalues
//-------------------------------------------------------------------------
Closure *g_closure_head = 0;
Closure *g_closure_tail = 0;

void push_closure()
{
	Closure *c = ALLOCATE(Closure, TAG_LOCALS, "new_closure:1");
	memset(c, 0, sizeof(Closure));
	if (g_closure_tail) {
		g_closure_tail->next = c;
		c->prev = g_closure_tail;
		g_closure_tail = c;
	} else {
		g_closure_tail = c;
		g_closure_head = c;
	...
	for (i = 0; i < current_number_of_locals; i++) {
		LocalVal *l = &(c->parent_locals[i]);
		l->index = locals_ptr[i]->dn.local_num;
		l->type = type_of_locals_ptr[i];
		l->name = locals_ptr[i]->name;
	}
	c->parent_local_num = current_number_of_locals;
}

void pop_closure()
{
	Closure *c = g_closure_tail;
	if (c) {
		g_closure_tail = c->prev;
		if (g_closure_tail) g_closure_tail->next = 0;
		else g_closure_head = 0;
#ifdef DEBUG
		printf("free closure:\n");
		dump_closure(c);
#endif
...
{
	return g_closure_tail;
}

Closure *parent_closure(Closure *c)
{
	if (!c) return 0;
	return c->prev;
}

UpVal *add_upvalue_toclosure(Closure *c, char *name, int type)
{
	if (!c) return 0;
	UpVal *uv = &(c->upvalues[c->upvalue_num]);
	if (c->upvalue_num == MAX_NUM_UPVALUES) {
		yyerror("upvalue number limit");
		return 0;
	}
	uv->name = name;
	uv->type = type;
	uv->index = c->upvalue_num;
	c->upvalue_num ++;

	// add upvalue to the stack
	LocalVal *lv = find_local_in_parent_closure(c, name);
	// if the parent's closure has no variable named 'name'
	// then add it as upvalue in the parent's closure
	if (!lv) {
		if (!c->prev) {yyerror("upvalue not exist"); return NULL;}
		add_upvalue_toclosure(c->prev, name, type);
	}
		
	/*
	printf("add upvalue %s:", name);
	dump_closure(c);
	*/
	return uv;

}
UpVal *add_upvalue(char *name, int type)
{
	Closure *c = current_closure();
	if (!c) {
		char buf[256];
		sprintf(buf, "add_upvalue %s:closure stack is null", name);
		yyerror(buf);
		return 0;
	}
	return add_upvalue_toclosure(c, name, type);
}

void dump_closure(Closure *c)
{
	if (!c) return;
	printf("{\n");
	int i;
	for (i=0; i<c->upvalue_num; i++) {
		...
		printf("upvalue %s, index %d\n", uv->name, uv->index);
	}
	printf("}\n");
}

UpVal *find_upvalue_inclosure(Closure *c, char *name)
{
	if (!c) return 0;
	int i;
	for (i=c->upvalue_num -1; i >= 0; i--) {
		UpVal *uv = &(c->upvalues[i]);
...
// if the name in the parent's local
LocalVal *find_local_in_parent_closure(Closure *c, char *name)
{
	if (!c) return 0;
	int i;
	for (i = c->parent_local_num -1; i >= 0; i--) {
		LocalVal *lv = &(c->parent_locals[i]);
...
UpVal *find_upvalue(char *name)
{
	Closure *c = current_closure();
	if (!c) {
		char buf[256];
		sprintf(buf, "find_upvalue %s:closure stack is null", name);
		yyerror(buf);
		return 0;
	}
	return find_upvalue_inclosure(c, name);
}
//-------------------------------------------------------------------------
// locals 
//-------------------------------------------------------------------------
...
void init_locals()
{
	...
	locals = CALLOCATE(CFG_MAX_LOCAL_VARIABLES, ident_hash_elem_t *, TAG_LOCALS, "init_locals:2");
	runtime_locals = CALLOCATE(CFG_MAX_LOCAL_VARIABLES, char, TAG_LOCALS, "init_locals:3");
...
void deactivate_current_locals()
{
	...
	for (i = 0; i < current_number_of_locals; i++) {
		...
		locals_ptr[i]->dn.upvalue_num = locals_ptr[i]->dn.local_num;
		locals_ptr[i]->dn.upvalue_type = type_of_locals_ptr[i];
		locals_ptr[i]->dn.local_num =  - 1;
	}
}
...
void reallocate_locals()
{
	...
	type_of_locals = RESIZE(type_of_locals, type_of_locals_size += CFG_MAX_LOCAL_VARIABLES, unsigned short, TAG_LOCALS, "reallocate_locals:1");
	type_of_locals_ptr = type_of_locals + offset;
	offset = locals_ptr - locals;
	locals = RESIZE(locals, locals_size, ident_hash_elem_t *, TAG_LOCALS, "reallocate_locals:2");
	locals_ptr = locals + offset;
	runtime_locals = RESIZE(runtime_locals, locals_size, char, TAG_LOCALS, "reallocate_locals:3");
...
void copy_variables(program_t *from, int type)
{
	...
	{
		copy_variables(from->inherit[i].prog, type | from->inherit[i].type_mod);
	}
...
/* copy a fun verbatim into this object, and possibly add it to the
list of functions in this object, as well
 */
void copy_function(program_t *prog, int index, program_t *defprog, int defindex, int typemod)
{
	ident_hash_elem_t *ihe;
	...
	/* add the identifier */
	ihe = find_or_add_ident(defprog->function_table[defindex].name, FOA_GLOBAL_SCOPE);
...
//-------------------------------------------------------------------------
// by chenyh 2016.9.12
// 检查是否即成的object里面是否有已定义的一样的class
int lookup_inherit_class(const char *clsname, ident_hash_elem_t **ihes, int ihenum)
{
    int i = 0, j = 0;
    inherit_t *inh;
    program_t *prog;
    ...
    for ( i = 0; i < NUM_INHERITS; i++) {
        prog = INHERIT(i)->prog;
        for (j = 0; j < prog->num_classes; j++) {
            str = prog->strings[prog->classes[j].name];
            if (strcmp(str, clsname) != 0) {
                continue;
            }
            if (ihenum != prog->classes[j].size) {
                // fprintf(stderr, "ihenum=%d,size=%d", ihenum, prog->classes[j].size);
                return 0;
            }
            // defined ambiguity in inherit object
            // 这段代码有bug, 需要找到class name的正确index by chenyh
            /* while (ihenum--) { */
            /*     if (strcmp(prog->strings[prog->class_members[ihenum].name], ihes[ihenum]->name) != 0) { */
            /*         fprintf(stderr, "ihenum=%d,ihename=%s,name=%s", ihenum, prog->strings[prog->class_members[ihenum].name], ihes[ihenum]->name); */
            /*         return 0; */
            /*     } */
            /* } */
            return 1;
...
/* Overload the fun index with the new definition */
void overload_function(program_t *prog, int index, program_t *defprog, int defindex, int oldindex, int typemod)
{
	int alias;
	...
	/* check that we aren't overloading a nomask fun */
	if (!(newflags &NAME_NO_CODE) && REAL_FUNCTION(oldflags) && (oldflags &NAME_NO_MASK) )
	{
		char buf[256];
		char *end = EndOf(buf);
		char *p;

		p = strput(buf, end, "Illegal to redefine 'nomask' fun \"");
	 ...
	 * Warn them about the behavior of inheriting the same fun
	 * from two branches.
	 *
	 * Note that we don't want to scream now, b/c if the fun is
	 ...
	 */
	if ( (pragmas &PRAGMA_WARNINGS) && REAL_FUNCTION(oldflags) && !(newflags &NAME_NO_CODE) && (oldflags &NAME_UNDEFINED) /* not defined at top level yet */
	)
	{
	...
	/* A new fun also has to be inserted, since this spot will be
	 * used when this fun is called in an object beneath us.  Point
	 * it at the overloaded fun.
	...
	/* The rule here is that the latest fun wins, so if it's not
	defined at this level and defined in the new object, we copy it in */
	if ( (oldflags &NAME_UNDEFINED) && (!(newflags &NAME_NO_CODE) ) )
	{
		int f = (prog->function_flags[index] &NAME_MASK) | NAME_DEF_BY_INHERIT | NAME_UNDEFINED;
		...
		if (FUNCTION_PROG(oldindex) == 0)
		{
			...
			the old fun table entry for removal */
...
/*
 * Copy all fun definitions from an inherited object. They are added
 ...
 *
 * A call to an inherited fun will not be
 * done through this entry (because this entry can be replaced by a new
 * definition). If an fun defined by inheritance is called, then one
...
int copy_functions(program_t *from, int typemod)
{
	...
	if (from->num_functions_defined && (from->function_table[from->num_functions_defined - 1].name[0] == APPLY___INIT_SPECIAL_CHAR) )
	{
	...
	for (i = 0; i < num_functions; i++)
	{
		...
		DEBUG_CHECK(func_entry->def.f_index >= prog->num_functions_defined, "Index of founction out of bounds!");
		funp = prog->function_table + func_entry->def.f_index;

		ihe = lookup_ident(funp->name);
		if (ihe && ( (num = ihe->dn.function_num) !=  - 1) )
		{
			/* The fun has already been defined in this object */
			overload_function(from, i, prog, func_entry->def.f_index, num, typemod);
		}
		else
		{
...
/*
 * Patch a fun definition of an inherited fun, to what it really
 * should be.
 * The name of the fun can be one of:
 *    object::name
 *    ::name
 * Where 'object' is the name of the superclass.
 *
 * Note: this fun is now only used for resolving :: references
 */
...
int arrange_call_inherited(char *name, parse_node_t *node)
{
	...
	if ( (shared_string = findstring(real_name) ) )
	{
		...
		for (; num_inherits > 0; ip++, num_inherits--)
		{
			...
			if (super_name)
			{
				...
				if (strncmp(super_name, ip->prog->name + l - 2-super_length, super_length) != 0 || !( (l - 2 == super_length) || ( (ip->prog->name + l - 3-super_length)[0] == '/') ) )
				{
					continue;
				}
			}
	...
	} /* if in shared string table */
	{
		...
		p = strput(buff, end, "No such inherited fun ");
		p = strput(p, end, name);
		yyerror(buff);
	}

	invalid: node->kind = NODE_CALL_2;
	node->v.number = F_CALL_INHERITED;
	node->l.number = 0;
	node->type = TYPE_ANY;
...
/*
 * Define a new fun. Note that this fun is called at least twice
 * for all fun definitions. First as a prototype, then as the real
 * fun. Thus, there are tests to avoid generating error messages more
 * than once by looking at (flags & NAME_PROTOTYPE).
 */
/* Warning: returns an index into A_COMPILER_FUNCTIONS, not the full
 * fun list
 */
int define_new_function(char *name, int num_arg, int num_local, int flags, int type)
{
	...
	if (runtime_num >= 0) {
		...
		/*
		 * The fun was already defined. It may be one of several
		 * reasons:
		 *
		 * 1.	There has been a prototype.
		 * 2.	There was the same fun defined by inheritance.
		 * 3.	This fun has been called, but not yet defined.
		 * 4.	The fun is doubly defined.
		...
		if (!(funflags &NAME_UNDEFINED) && !(flags &NAME_PROTOTYPE) ) {
			...
			p = strput(buff, end, "Redeclaration of fun ");
			p = strput(p, end, name);
			p = strput(p, end, ".");
			yyerror(buff);
			return  - 1;
		 ...
		 * fun. In both cases, we now consider this to be THE new
		 ...
		 * this fun really has been defined already.
		 *
		 * 'nomask' functions may not be redefined.
		 */
		if ( (funflags &NAME_NO_MASK) && !(funflags &NAME_PROTOTYPE) && !(flags &NAME_PROTOTYPE) ) {
			char buf[256];
			char *end = EndOf(buf);
			char *p;
		...
		overloading a fun must have the same signature */
		if (exact_types && (funflags &NAME_PROTOTYPE) && funp->type != TYPE_UNKNOWN) {
			int i;

			/* This should be changed to catch two prototypes which disagree */
			if (!(flags &NAME_PROTOTYPE) ) {
				if (fundefp->num_arg != num_arg && !(funflags &NAME_VARARGS) ) {
					yyerror("Arguments number disagrees with previous definition.");
				}
				
				if (!(funflags &NAME_STRICT_TYPES) ) {
					yyerror("Called fun n0ot compiled with type testing.");
				}
		 ...
		 * prototype), clear it out.  Don't free the fun name, though;
	...
	if (exact_types && num_arg) {
		*( (unsigned short*)mem_block[A_ARGUMENT_INDEX].block + num) = mem_block[A_ARGUMENT_TYPES].current_size / sizeof(unsigned short);
		add_to_mem_block(A_ARGUMENT_TYPES, (char*)type_of_locals_ptr, num_arg *sizeof(*type_of_locals_ptr) );
	}
	return num;
}
...
int define_variable(char *name, int type, int hide)
{
	variable_t *dummy;
	int n;
	ident_hash_elem_t *ihe;
...
char *compiler_type_names[] = 
{
	"unknown", "mixed", "void", "void", "int", "string", "object", "mapping", "fun", "float", "buffer",
};

/* This routine has the semantics of strput(); see comments in simulate.c */
char *_get_type_name(char *where, char *end, int type)
{
	int pointer = 0;
...
void free_prog_string(short num)
{
	...
	for (prv =  - 1, i =  *idxp; i != num; prv = i, i = next_tab[i])
	{
		if (i ==  - 1)
		{
			...
			yyerror("string not in prog table.\n");
			return ;
		}
	}
...
int validate_function_call(int f, parse_node_t *args)
{
	...
	if (funflags &NAME_HIDDEN)
	{
		...
		p = strput(buf, end, "Illegal to call private fun '");
	...
	/*
	 * Verify that the fun has been defined already.
	 */
	if ( (funflags &NAME_UNDEFINED) && !(funflags &(NAME_PROTOTYPE | NAME_DEF_BY_INHERIT) ) && exact_types)
	{
		char buf[256];
		char *end = EndOf(buf);
		char *p;
	 ...
	 */
	if (!(funflags &NAME_VARARGS) && (funflags &NAME_STRICT_TYPES) && exact_types)
	{
		char buff[256];
		char *end = EndOf(buff);
		char *p;
		...
		if (num_var)
		{
			p = strput(buff, end, "Illegal to pass a variable number of arguments to non-varargs fun ");
			p = strput(p, end, funp->name);
			p = strput(p, end, "\n");
			yyerror(buff);
		}
...
program_t *compile_file(int f, char *name)
{
	 ...
	 */
	if (guard) {
		error("Object can't be loaded during compilation.\n");
	}
	guard = 1;
...
INLINE void copy_in(int which, char **start)
{
	char *block;
	int size;
...
static void copy_and_sort_function_table(program_t *prog, char **p)
{
	int *temp,  *inverse;
	int i, num_runtime;
	...
	num_runtime = mem_block[A_RUNTIME_FUNCTIONS].current_size / sizeof(runtime_function_u);
	#ifdef COMPRESS_FUNCTION_TABLES
		{
			...
			for (i = 0; i < n_ov; i++)
			{
				...
				{
					/* remember the fun entries have 'moved' */
			...
			for (i = 0; i < n_def; i++)
			{
				...
				if (!(FUNCTION_FLAGS(ri) &NAME_INHERITED) )
				{
					...
					DEBUG_CHECK(oldix >= num, "Function index out of bounds");
	...
	*p += align(new_num *sizeof(compiler_function_t) );
	...
	/* Free fun entries that have been discarded */
...
//-------------------------------------------------------------------------
	...
	void compress_function_tables()
	{
		...
		while (f_def >= 0)
		{
			...
			if ( (FUNCTION_FLAGS(f_def) &NAME_INHERITED) && f_def == EXPECTED_INDEX(rfu) )
			{
				break;
			}
			f_def--;
		...
		else if (f_def)
		{
			...
			mem_block[A_RUNTIME_FUNCTIONS].current_size = (j + n_def) *sizeof(runtime_function_u);
		}
	}
#endif 
...
static program_t *epilog()
{
	...
	/*
	 * Define the __INIT fun, but only if there was any code
	 * to initialize.
	 */
	UPDATE_PROGRAM_SIZE;
	...
	if (mem_block[A_INITIALIZER].current_size) {
		...
		fun = define_new_function(APPLY___INIT, 0, 0, NAME_STRICT_TYPES | NAME_PRIVATE, TYPE_VOID);
	 ...
	 * so that only one fun definition has to be fooled with for
	...
	for (i = 0; i < num_fun; i++) {
		...
		if ( (funflags &NAME_UNDEFINED) && (funflags &NAME_DEF_BY_INHERIT) ) {
			/*If it's a real fun, make it a real fun at this level*/
		...
		if (funflags &NAME_ALIAS) {
			...
			if (!(FUNCTION_FLAGS(which) &NAME_INHERITED) || FUNCTION_ALIAS(which) >= 2) {
	...
	*prog = NULL_program;
	...
	memcpy( ( (char*) &prog->file_info[2]), mem_block[A_FILE_INFO].block, mem_block[A_FILE_INFO].current_size);
	...
	if (p - (char*)prog != size) {
		...
		fprintf(stderr, "is: %ld, expected: %i\n", (long) (p - (char*)prog), size);
	}
#endif 
#ifdef BINARIES
...
void prepare_cases(parse_node_t *pn, int start)
{
	...
	{
		quickSort( (char*)ce_start, ce_end - ce_start, sizeof(parse_node_t*), (int (*)(void *, void *))string_case_compare);
	}
	else
	{
		quickSort( (char*)ce_start, ce_end - ce_start, sizeof(parse_node_t*), (int (*)(void *, void *))case_compare);
	}
	...
	while (ce < ce_end)
	{
		...
		if (pn->kind == NODE_SWITCH_RANGES && this_key <= last_key)
		{
			...
			translate_absolute_line( (*ce)->line, (unsigned short*)mem_block[A_FILE_INFO].block,  &fi1, &l1);
			translate_absolute_line( (*(ce - 1) )->line, (unsigned short*)mem_block[A_FILE_INFO].block,  &fi2, &l2);
			f1 = PROG_STRING(fi1);
			f2 = PROG_STRING(fi2);
