	...
	#define IS_CLASS(t) ((t & TYPE_MOD_CLASS) == TYPE_MOD_CLASS)
...
//------------------------ upvalues ----------------------------
#define MAX_NUM_UPVALUES CFG_MAX_LOCAL_VARIABLES
...
typedef struct Closure {
	struct Closure *next;
	struct Closure *prev;
	UpVal upvalues[MAX_NUM_UPVALUES];
	int upvalue_num;
	LocalVal parent_locals[CFG_MAX_LOCAL_VARIABLES]; // locals in the parent closure
	int parent_local_num;
} Closure;
...
//--------------------------------------------------------------
	...
	char *_get_type_name(char *, char *, int);
    ...
    int lookup_inherit_class(const char *, ident_hash_elem_t **, int);
	...
	parse_node_t *throw_away_mapping(parse_node_t*);
		...
		INLINE_STATIC void realloc_mem_block(mem_block_t *, int);
		INLINE_STATIC void add_to_mem_block(int, char *, int);
		INLINE_STATIC void insert_in_mem_block(int, int, int);
		INLINE_STATIC char *allocate_in_mem_block(int, int);

		INLINE_STATIC void realloc_mem_block(mem_block_t *m, int size)
		{
		...
		INLINE_STATIC void add_to_mem_block(int n, char *data, int size)
		{
		...
		INLINE_STATIC char *allocate_in_mem_block(int n, int size)
		{
			mem_block_t *mbp = &mem_block[n];
			char *ret;
		...
		INLINE_STATIC void insert_in_mem_block(int n, int where, int size)
		{
			mem_block_t *mbp = &mem_block[n];
			char *p;
