...
/* ANSI/K&R compatibility stuff; ANSI/K&R
 * The correct way to prototype a fun now is:
 ...
 * vararg fun arguments under AIX
 */
...
/*
  ...
  overlaying system malloc with another fun also named malloc doesn't
...
!Only one malloc(SYSMALLOC, SMALLOC, BSDMALLOC)package should be defined 
#endif 
...
#include "malloc.h"
											   ...
											   !Sorry, You need to specify a malloc package in local_options / options.h 
#endif 
...
char *xalloc(int);
#ifdef DEBUGMALLOC
char *int_string_copy(const char *, char*);
char *int_string_unlink(char *, char*);
char *int_new_string PROT( (int, char*) );
char *int_alloc_cstring(char *, char*);
#else 
char *int_string_copy(const char*);
char *int_string_unlink(char*);
