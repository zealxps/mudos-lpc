...
#define PUSH_STRING    (0 << 5)
#define PUSH_NUMBER    (1 << 5)
#define PUSH_GLOBAL    (2 << 5)
#define PUSH_LOCAL     (3 << 5)
#define PUSH_UPVALUE   (4 << 5)

#define PUSH_WHAT      (7 << 5)
 ...
 * when the current fun is defined by inheritance.
...
typedef struct
{
	...
	short caller_type; /* was this a locally called fun? */
} control_stack_t;
...
void push_refed_class(array_t *, unsigned char);
