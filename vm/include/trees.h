...
#ifdef DBVAR_NOASSIGN
	#define DBVAR_NODE_CLEAR(node) {(node)->dbvar_noassign = 0;}
	#define DBVAR_NODE_SET(node, type) {(node)->dbvar_noassign = ((type) & NAME_DBVAR_NOASSIGN) ? 1 : 0;}
	#define DBVAR_NODE_GET(node) (node)->dbvar_noassign
#else
	#define DBVAR_NODE_CLEAR(node)
	#define DBVAR_NODE_SET(node, type)
	#define DBVAR_NODE_GET(node) 0
#endif
