	 ...
	 *    with the address of the fun in the program. Inherited functions
	 *    will be found here too, with information of how far up the inherit
	 *    chain that the fun was defined.
	...
	/*
	 * When a new object inherits from another, all fun definitions
	 ...
	 * types are saved of the arguments for that fun during compilation.
	...
	/* NAME_INHERITED - The fun entry that exists in this object actually
	is a fun in an object we inherited
	 * NAME_UNDEFINED - the fun hasn't been defined yet at this level
	 * NAME_STRICT_TYPES - compiled with strict type testing
	 * NAME_PROTOTYPE - only a prototype has been found so far
	 * NAME_DEF_BY_INHERIT - this fun actually exists in an object we've
	inherited; if we don't find a fun at this level
	we'll use that one
	 * NAME_ALIAS     - This entry refers us to another entry, usually because
	this fun was overloaded by that fun
	 */
	...
	#define NAME_STATIC			0x0200	/* Static fun or variable */
...
// 就不用定义新的关键字了，复用NAME_VARARGS 作为dbvar的限定。
#ifdef DBVAR_NOASSIGN
	#define NAME_DBVAR_NOASSIGN		0x4000	
	#define NAME_TYPE_MOD		(NAME_HIDDEN | NAME_STATIC | NAME_NO_MASK | NAME_PRIVATE | NAME_PROTECTED | NAME_PUBLIC | NAME_VARARGS|NAME_DBVAR_NOASSIGN)
#else
	...
	typedef struct
	{
		...
		unsigned short f_index; /* Index in sorted fun table */
	} runtime_defined_t;
	...
	typedef struct program_s
	{
		...
		int heart_beat; /* Index of the heart beat fun. -1 means
		 * no heart beat */
		/*
		 * The types of fun arguments are saved where 'argument_types'
		 * points. It can be a variable number of arguments, so allocation is
		 * done dynamically. To know where first argument is found for fun
		 * 'n' (number of fun), use 'type_start[n]'. These two arrays will
	...
	runtime_function_u *find_func_entry(program_t *, int);
