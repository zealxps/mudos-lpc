/* Satoria's malloc intended to be optimized for QTZ_ServerEngine.
...
POINTER smalloc_malloc(size_t size)
{
	/* int i; */
	u *temp;
...
SFREE_RETURN_TYPE smalloc_free(POINTER ptr)
{
	u *block;
	u i;
...
{
	printf("[%c%d: %d]  ", (*ptr &THIS_BLOCK ? '+' : '-'), (int)ptr, (int) (*ptr &MASK) );
}
...
//-------------------------------------------------------------------------
	...
	static free_block_t *free_tree = &dummy2;
		...
		/* this fun returns a value so that it can be used in ,-expressions. */
		static int do_check_avl()
		{
			...
			if (inconsistency)
			{
				fflush(stderr);
				fflush(stdout);
				fatal("Inconsistency could crash the OS\n");
			}
			return 0;
		}
	#endif /* DEBUG_AVL */
			...
			for (;;)
			{
				...
				else
				{
					...
					if (q = p->left)
					{
						 ...
						 * will handle q very inefficient throughout the fun
						 * large_malloc()
						 */
						for (;;)
						{
...
POINTER smalloc_realloc(POINTER p, size_t size)
{
	unsigned *q, old_size;
	char *t;
...
//-------------------------------------------------------------------------
	...
	void show_mstats(outbuffer_t *ob, char *s)
	{
		...
		outbuf_addv(ob, "unused from current chunk          %10d (g)\n\n", unused_size);
		outbuf_addv(ob, "    Small blocks are stored in small chunks, which are allocated as\n");
 ...
 */
POINTER smalloc_calloc(size_t nelem, size_t sizel)
{
	char *p;
...
//-------------------------------------------------------------------------
	...
	static void walk_new_small_malloced(func)void(*func) (POINTER, int);
	{
		int i;
		u *p,  *q;
