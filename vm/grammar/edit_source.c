 ...
 */
#define OPTIONS_INCL	  "grammar/options_incl.h"
#define PACKAGES		  "packages/packages"
#define OPTIONS_H		  "grammar/options.h"
#define DEFAULT_LOCAL_OPTIONS	  "grammar/local_options.h"
#define OPTION_DEFINES	  "grammar/option_defs.c"
#define FUNC_SPEC		  "grammar/func_spec.c"
#define FUNC_SPEC_CPP	  "grammar/func_spec.cpp"
#define EFUN_TABLE		  "grammar/efunctions.h"
#define OPC_PROF		  "grammar/opc.h"
#define OPCODES			  "grammar/opcodes.h"
#define EFUN_PROTO		  "grammar/efun_protos.h"
#define EFUN_DEFS		  "grammar/efun_defs.c"

#define PRAGMA_NOTE_CASE_START 1

char LOCAL_OPTIONS[100] = DEFAULT_LOCAL_OPTIONS;
char PACKAGE_PATH[100] = "packages";
...
static void handle_define()
{
	  ...
	  while ( isalunum ( *tmp ) )
	  {
			if ( q < end ) *q++ = *tmp++;
				  ...
				  yyerror ( "Macro name is too long.\n" );
	  }
	  if ( q == namebuf )
...
static int expand_define()
{
	  ...
	  else
	  {
			...
			if ( n != p->nargs )
			{
				  yyerror ( "Wrong number in macro arguments" );
				  return 0;
			}
			/* Do expansion */
			b = buf;
				  ...
				  if ( *e == MARKS )
				  {
						...
						else
						{
							  for ( q = args[*e++ - MARKS - 1]; *q; )
							  {
									*b++ = *q++;
									if ( b >= buf + DEFMAX )
										  yyerror ( "Macro expansion is overflow" );
							  }
						}
				  }
				  else
			...
			for ( p = defns[i]; p; p = p->next )
				  if ( ! ( p->flags & DEF_IS_UNDEFINED ) )
				  {
						...
						if ( strncmp ( p->name, "PACKAGE_", 8 ) ==0 )
						{
							  ...
							  if ( num_packages >= 100 )
							  {
									fprintf ( stderr, "Too many packages then 100.\n" );
									exit ( -1 );
							  }
				  ...
				  *yyp = 0;
				  ...
				  else if ( !strcmp ( "if", yyp ) )
				  {
						...
						if ( *outp != '\n' )
							  ...
							  yyerrorp ( "Condition is too complex in %cif" );
...
#define NUM_FILES	  5
	  ...
	  for ( i = 0; i < efun_code; i++ )
	  {
			...
			fprintf ( files[3],"void f_%s (void);\n", efun_names[i] );
			// fprintf ( stderr, "efun_code[%d] %s.\n", i, efun_names[i]);
	  }
	  fprintf ( files[0], "};\n" );
	  fprintf ( files[1], "};\n" );

	  fprintf ( stderr, "You have efuns. efun1_code[%d], op_code[%d], efun_code[%d].\n", efun1_code, op_code, efun_code);
	  if ( efun1_code + op_code >= 256 )
	  {
			fprintf ( stderr, "You have way too many efuns. efun1_code[%d] + op_code[%d] > 255.\n", efun1_code, op_code );
		  	exit(-1);
	  }
	  if ( efun_code >= 256 )
	  {
			fprintf ( stderr, "You have way too many efuns. efun_code[%d] >= 256.\n", efun_code );
		  	exit(-1);
	  }
...
static void handle_local_defines()
{
	  ...
	  /* undefine _OPTIONS_H_ so it doesn't get propagated to the QTZ_SE
	  ...
	  if ( problem )
	  {
		  fprintf(stderr, "\
				  ...
				  ***earlier version of the engine.Please lookup the new options\n\
...
static void handle_build_func_spec ( char *  command )
{
	  ...
	  fprintf (stderr, "sys cmd:%s\n", buf);
	  system ( buf );
	  for ( i = 0; i < num_packages; i++ )  {
			sprintf ( buf, "%s -I. %s/%s_spec.h >>%s",
					  command, PACKAGE_PATH, packages[i], FUNC_SPEC_CPP );
	  fprintf (stderr, "sys cmd:%s\n", buf);
			system ( buf );
	  }
...
{
// 需要事先编辑applies文件
// 在我们现在应用中，并不需要事先绑定这些 APPLY_*，所以就不用自动生成了 -- canoe
#if 0	
...
static void handle_malloc()
{
	...
	if ( lookup_define ( "SYSMALLOC" ) )
		the_malloc = "grammar/sysmalloc.c";
	if ( lookup_define ( "SMALLOC" ) )
		the_malloc = "grammar/smalloc.c";
	if ( lookup_define ( "BSDMALLOC" ) )
		the_malloc = "grammar/bsdmalloc.c";

	if ( lookup_define ( "WRAPPEDMALLOC" ) )
		the_wrapper = "grammar/wrappedmalloc.c";
	if ( lookup_define ( "DEBUGMALLOC" ) )
		the_wrapper = "grammar/debugmalloc.c";
	  ...
	  if ( unlink ( "src/malloc.c" ) == -1 && errno != ENOENT )
			perror ( "unlink src/malloc.c" );
	  if ( unlink ( "src/mallocwrapper.c" ) == -1 && errno != ENOENT )
	  ...
	  else
	  {
			...
			if ( link ( "plainwrapper.c", "mallocwrapper.c" ) == -1 )
				  perror ( "link src/mallocwrapper.c" );
			*/	  
	  }
	  if ( link ( the_malloc, "src/malloc.c" ) == -1 )

			perror ( "link src/malloc.c" );
}
							...
							const char *before,const char *after )
{
	  ...
	  printf ( "Checking for include file <%s> ,please wait... ", file );
...
static int check_library (const char *  lib)
{
	  ...
	  printf ( "Checking for library %s,please wait ... ", lib );
	  ct = fopen ( "comptest.c", "w" );
							...
							char *	type, char *  func )
{
	  ...
	  printf ( "Checking return type of %s(),please wait...", func );
	  ct = fopen ( "comptest.c", "w" );
	  fprintf ( ct, "%s\n\n%s%s();\n", pre, type, func );
	  fclose ( ct );
	  ...
	  if ( !system ( buf ) )
	  {
			...
			printf ( "type return is %s\n", type );
			return 1;
	  }

	  printf ( "Type does not return %s\n", type );
	  return 0;
}
#endif
...
static void find_memmove()
{
	  ...
	  printf ( "Checking for memmove(),please wait..." );
	  if ( check_memmove ( 0, "" ) )
	  {
			printf ( " exists\n" );
			return;
	  ...
	  printf ( " missing; using QTZ_SE's version\n" );
...
static void handle_configure()
{
	...
	// merge from mudos v22.14 by canoe
	if ( 0 && check_prog("BSD_READDIR", "#include <sys/dir.h>", "struct direct *d; d->d_namlen;", 0)) {
	...
	// merge from mudos v22.b.14 by canoe
	...
	printf ( "QTZ_SE:Checking for random number generator ..." );
	if ( check_prog ( "RANDOM", 0, "srandom(0);", 0 ) ) {
		printf ( "QTZ_SE:Using random()\n" );
	} else if ( check_prog ( "DRAND48", 0, "srand48(0);", 0 ) ){
		printf ( "QTZ_SE: Using srand()\n" );
	} else if ( check_prog ( "RAND", 0, "srand(0);", 0 ) ){
		printf ( "QTZ_SE: Using rand()\n" );
	} else{
		printf ( "QTZ_SE:WARNING: Can't not find a random number generator\n" );
		exit ( -1 );
	}

	if ( check_prog ( "USE_BSD_SIGNALS", 0, "SIGCHLD; wait3(0, 0, 0);", 0 ) ) {
		printf ( "QTZ_SE:Using BSD signals.\n" );
	} else	{
	...
	fprintf ( yyout, "#define SIZEOF_INT %li\n", sizeof ( int ) );
	fprintf ( yyout, "#define SIZEOF_PTR %li\n", sizeof ( char * ) );
	fprintf ( yyout, "#define SIZEOF_SHORT %li\n", sizeof ( short ) );
	fprintf ( yyout, "#define SIZEOF_FLOAT %li\n", sizeof ( float ) );
	...
	if ( sizeof ( unsigned long ) == 4 )
		...
		{
			printf ( "WARNING: Can't find a 32 bit integral type.\n" );
			exit ( -1 );
		}
...
int main ( int	argc, char **  argv )
{
	  ...
	  while ( idx < argc )
	  {
			...
			else if (strcmp(argv[idx], "-package_path" ) ==0)
			{
				char *path = argv[++idx];
				fprintf(stderr, "pathlen=%ld, %s\n", strlen(path), path);
				if (strlen(path) > 0) {
					// sprintf(LOCAL_OPTIONS, "%s/local_options.h", path);
					sprintf(PACKAGE_PATH, "%s", path);
				}
			}
			else
			{
