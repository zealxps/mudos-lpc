 ...
 */
int arg_types[512], last_current_type;
...
{
       0,    64,    64,    64,    66,    66,    68,    70,    70,    72,
      88,    88,    90,    91,    97,   104,   186,   186,   188,   206,
     207,   208,   210,   218,   228,   229,   230
};
#endif
...
yyreduce:
  ...
  case 15:
	...
	if (min_arg == -1)
        ...
        if (min_arg > 5) mf_fatal("min_arg > 5\n");
	...
	if (i == last_current_type) {
		...
		if (last_current_type == NELEMS(arg_types)) {
	        fprintf ( stderr, "last_current_type = %d\n", last_current_type);
	        fprintf ( stdout, "A last_current_type = %d, i=%d, %lu, true=%d\n", last_current_type, i, NELEMS(arg_types), last_current_type == NELEMS(arg_types));
		    yyerror("Array 'arg_types' is too small");
	        fprintf ( stdout, "B last_current_type = %d, %lu\n", last_current_type, NELEMS(arg_types));
}
	    }
	}
...
int yylex() {
	...
	case '!':
	{
	    ...
	    fgets(buff, 2047, yyin);
	 	fprintf(stderr, "QTZ_SE:Configuration problem: %s\n", buff);
	    exit(-1);
	}
	case '#':
	{
